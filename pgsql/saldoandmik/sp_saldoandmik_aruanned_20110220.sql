...
/*
select sp_saldoandmik_aruanned(rekv.id, date(2011,03,31), 2011, 3, 0, 1) from rekv where id = 63

delete from TMP_SK_ARUANNED

select * from TMP_SK_ARUANNED

select sp_saldoandmik_aruanned(63, date(2011,01,31), 2011, 2, 0, 0)

			select sum(ifnull(kr,0)) - sum(ifnull(db,0)) 
			from saldoandmik
			where aasta = 2011 and kuu = 1 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, 64) > 9) or  saldoandmik.rekvid = 64)
			and val(left(ltrim(rtrim(saldoandmik.konto)),1)) >= 3 and val(left(ltrim(rtrim(saldoandmik.konto)),2)) <= 64;

select * from rekv where parentid = 64

select sum(db), sum(kr), sum(kr)-sum(db) from (
select * from saldoandmik 
where kuu = 1 and aasta = 2011 and val(left(ltrim(rtrim(saldoandmik.konto)),1)) >= 3 and val(left(ltrim(rtrim(saldoandmik.konto)),2)) <= 64 and rekvid in (64,124)
) tmp

select * from saldoandmik where rekvid = 124 and kuu = 1 and aasta = 2011 and konto like '6%'

SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, 63) > 3
;
sp_saldoandmik_aruanned
...
--	raise notice 'K 103500 %',lnKreedit;
	...
	insert into  tmp_sk_aruanned (konto,nimetus,summa01,  kpv, rekvid, timestamp, tyyp) 
			select '10', 'Kдibevara' ,ifnull(sum(db) - sum(kr),0) - ifnull(lnDeebet,0) + ifnull(lnKreedit,0),tdKpv, tnRekvId,lcReturn, 0 from saldoandmik 
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)
			and konto like '10%'; 
...
--Saldoandmikust (Sum: Kontod 1035* Deebet) - (Sum: Kontod 1035* Kreedit) - konto 103500 deebet + konto 103500 kreedit
	...
	insert into  tmp_sk_aruanned (konto,nimetus,summa01,  kpv, rekvid, timestamp, tyyp) 
			select '1035', 'Nхuded toetuste ja siirete eest' ,ifnull(sum(db) - sum(kr),0) - IFNULL(lnDeebet,0) + IFNULL(lnKreedit,0), tdKpv, tnRekvId,lcReturn, 0 from saldoandmik 
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)
