...
update saldoandmik set aasta = 3011 where aasta = 2011 and kuu = 10

select * from aa where arve = 'TP' AND PARENTID  IN (select id from rekv where parentid < 200)
*/
...
raise notice 'jooksev aasta 2035+2535:K %',lnKr31;
raise notice 'jooksev aasta 2035+2535:D %',lnDb31;
...
*/
			select sum(kr), sum(db) into lnKreedit,lnDeebet  
			from saldoandmik
			where aasta =  year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)
			and (left(ltrim(rtrim(saldoandmik.konto)),6)) in ('203500','203540');		

raise notice 'jooksev aasta 203500+203540:K %',lnKreedit;
raise notice 'jooksev aasta 203500+203540:D %',lnDeebet;


			lnKreedit = lnKr31 - ifnull(lnKreedit,0);	
			lnDeebet = lnDb31 - ifnull(lnDeebet,0);	
			lnDeebet = lnKreedit - lnDeebet;

raise notice 'jooksev aasta K kokku %',lnDeebet;
...
raise notice 'eelmine aasta 2035, 2535K  %',lnKr7;
raise notice 'eelmine aasta 2035, 2535D  %',lnDb7;
...
/*
			...
			lnDeebet = ifnull(lnDeebet,0);
*/
			select sum(kr),sum(db) into lnKr31,lnDb31  
			from saldoandmik
			where aasta = lnDb2 and kuu = lnDb3  
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)
			and (left(ltrim(rtrim(saldoandmik.konto)),6)) in ('203500','203540');		

raise notice 'eelmine aasta 203500, 203540K  %',lnKr31;
raise notice 'eelmine aasta 203500, 203540D  %',lnDb31;

			lnKreedit = (lnKr7 - ifnull(lnKr31,0)) - (lnDb7 - ifnull(lnDb31,0));

			lnKreedit = ifnull(lnKreedit,0);
			lnDeebet = ifnull(lnDeebet,0);

raise notice 'eelmine aasta Kokku  %',lnKreedit;
...
--(sum 257* kreedit (RV 01)) - (sum 257* kreedit RV 01))
	...
	insert into  tmp_sk_aruanned (konto,nimetus, summa01,  kpv, rekvid, timestamp, tyyp) values
			('71','  Materiaalse põhivara soetus',lnDeebet,tdKpv, tnRekvId,lcReturn, 0); 
	...
	if tnSvod = 1 and tnRekvId = 63 and (select count(*) from saldoandmik where rekvid = 999 and aasta = year(tdKpv) and kuu = month(tdKpv)) > 0 then
		...
		loop	
			...
			select sum(db) into lnDeebet from tmp_saldoandmik 
				...
				and left(ltrim(rtrim(tp)),4) = left(ltrim(rtrim(lcOmatp)),4) and left(konto,1) = '7'; 
			select sum(kr) into lnKreedit from tmp_saldoandmik 
				where ltrim(rtrim(timestamp)) = ltrim(rtrim(lcreturn)) and tyyp = 0 
				and left(ltrim(rtrim(tp)),4) = left(ltrim(rtrim(lcOmatp)),4) and left(konto,1) = '7'; 

				raise notice '7 Db: %',lnDeebet;
				raise notice '7 Kr: %',lnKreedit;
