/*
select * from library where kood = '35200'
select * from curJournal where kood5 = '35200'
 select sp_eelarve_aruanne4(63, date(2012,12,31), '%', '%', '%', 1, 1)

select tunnus, kood1, kood5, nimetus,sum(summa1/15.6466) as eelarve,sum(summa2/15.6466) as taitmine 
		 from tmp_eelproj_aruanne1 where timestamp = '201303200150462KOKKU'
		 and kood5 like '38%'
		 group by tunnus,kood1,kood5, nimetus
		 order by kood1,kood5
CO2
select sum(summa) from curJournal where kreedit = '103556' and kood5 like '15%' and aasta = 2012 
K 103556

select * from curJournal where 	 kreedit = '103556' and kood5 like '15%' and aasta = 2012
...
--rea 3500,352
	select sum(summa1) into lnSumma1 
	...
	select  sum(summa*kuurs) into lnSumma2 
		...
		and (kood5 like '3500.%' or kood5 like '352%') 		
		and deebet like '100%' and (kreedit not like '7%' and kreedit <> '352000');
	...
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		select '','3520','Saadavad toetused tegevuskuludeks',sum(summa1),sum(summa2),lcreturn,date(),tnrekvId,'001352'::varchar from tmp_eelproj_aruanne1
				where  timestamp = lcreturn
				and ltrim(rtrim(kood5)) in ('352.00.17.1','352.00.17.2','35200','35201','3500;352');

	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		select '','35200','Tasandusfond',sum(summa1),sum(summa2),lcreturn,date(),tnrekvId,'0013521'::varchar from tmp_eelproj_aruanne1
				where  timestamp = lcreturn
				and ltrim(rtrim(kood5)) in ('352.00.17.1','35200');
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		select '','35201','Toetusfond ',sum(summa1),sum(summa2),lcreturn,date(),tnrekvId,'0013522'::varchar from tmp_eelproj_aruanne1
				where  timestamp = lcreturn
				and ltrim(rtrim(kood5)) in ('352.00.17.2','35201');

	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		select '','382.388','Muud tegevustulud ',sum(summa1),sum(summa2),lcreturn,date(),tnrekvId,'0013899'::varchar from tmp_eelproj_aruanne1
				where  timestamp = lcreturn
...
-- kustutame ebavajaliku andmed
 delete from tmp_eelproj_aruanne1 
	where timestamp = lcreturn 
	and (
	kood5 in ('1032.1.','1551','20.5','2036')	
