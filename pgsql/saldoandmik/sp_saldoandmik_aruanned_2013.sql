...
/*
select sp_saldoandmik_aruanned(63, date(2012,06,30), 2012, 5, 0, 1)
...
--Kokku liikumised
	...
	insert into  tmp_sk_aruanned (konto, nimetus, summa01,summa02,summa03,summa04,summa05,summa06,summa07, kpv, rekvid, timestamp, tyyp)
	 select '19','Kokku liikumised',sum(summa01),sum(summa02),sum(summa03),sum(summa04),sum(summa05),sum(summa06),sum(summa07),tdKpv, tnRekvId,lcReturn, 0
			...
			where timestamp = lcReturn and tyyp = 0;
...
--Saldoandmikust (Sum: Kontod 4*kuni 64* Kreedit) - (Sum: Kontod 4* kuni 64* Deebet)
		...
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '5001', 'Avaliku teenistuse ametnike töötasu (va kaitseväelased)',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),4) = '5001';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '50010', 'Juhtide töötasu',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),5) = '50010';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '50014', 'Keskastme spetsialistide töötasu',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
		...
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '50027', 'Nooremspetsialistide ja assistentide töötasu',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),5) = '50027';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '50028', 'Tööliste töötasu',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),5) = '50028';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '5008', 'Muude isikute töötasud',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),4) = '5008';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '5009', 'Eripensionid ja pensionisuurendused',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),4) = '5009';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
...
--Saldoandmikust (Sum: Kontod 3*kuni 7* Kreedit) - (Sum: Kontod 3* kuni 7* Deebet)
		...
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '913', 'Muu informatsioon',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),3) = '913';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
			select '9130', 'Tähtajaks tasumata kohustused',ifnull(sum(kr) - sum(db),0),tdKpv, tnRekvId,lcReturn, 0 
			from saldoandmik inner join library on (library.kood = saldoandmik.konto AND library.library = 'KONTOD')
			where aasta = year(tdKpv) and kuu = month(tdKpv) 
			and (saldoandmik.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, lnRekvId) > lnTase) or  saldoandmik.rekvid = lnRekvId)			
			and left(ltrim(rtrim(konto)),4) = '9130';
		insert into  tmp_sk_aruanned (konto, nimetus, summa01,  kpv, rekvid, timestamp, tyyp) 
...
--				group by nimetus, konto , tegev , tp , allikas, rahavoo;
			...
			delete from tmp_saldoandmik where rekvid = tnRekvId and tyyp <> 1 or (db - kr = 0) or (left(ltrim(rtrim(konto)),1) = '9' and len(ltrim(rtrim(tp)))>6);
