...
DROP TABLE tmp_subkontod_report;

CREATE TABLE tmp_subkontod_report
(
  ...
  algjaak numeric(18,6) DEFAULT 0,
  db numeric(18,6) DEFAULT 0,
  kr numeric(18,6) DEFAULT 0,
  loppjaak numeric(18,6) DEFAULT 0,
  ...
  rekvid integer,
  subrekvid integer DEFAULT 0,
  id serial NOT NULL,
  subrekvnimi character varying(254)
)
WITH (
  OIDS=FALSE
);
ALTER TABLE tmp_subkontod_report
  OWNER TO vlad;
GRANT ALL ON TABLE tmp_subkontod_report TO vlad;
GRANT ALL ON TABLE tmp_subkontod_report TO public;
GRANT ALL ON TABLE tmp_subkontod_report TO dbvaatleja;
GRANT ALL ON TABLE tmp_subkontod_report TO dbkasutaja;
GRANT ALL ON TABLE tmp_subkontod_report TO dbpeakasutaja;
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION fnc_get_asutuse_staatus(integer, integer)
  OWNER TO vlad;
GRANT EXECUTE ON FUNCTION fnc_get_asutuse_staatus(integer, integer) TO vlad;
GRANT EXECUTE ON FUNCTION fnc_get_asutuse_staatus(integer, integer) TO public;
...
begin
	...
	if ifnull(lnCount,0) < 1 then
		...
		create table tmp_subkontod_report (id serial, asutusId int default 0, Asutus varchar(254) default space(1), regkood varchar(20) default space(1), 
			...
			algjaak numeric (18,6) default 0, db numeric(18,6) default 0, kr numeric(18,6) default 0, loppjaak numeric(18,6) default 0,
...
if tnSvod = 1 then
	...
	if tnOpt = 1 then
		...
		-- kaibed
			select sum(case when ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) then (summa) else 0::numeric(18,6) end) as db, 
				sum(case when ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)) then (summa) else 0::numeric(18,6) end) as kr into v_kaibed 
				from journal1 
			...
			select sum(case when ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) then (summa) else 0::numeric(18,6)  end) as db, 
				sum(case when ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)) then (summa) else 0::numeric(18,6) end ) as kr into v_saldo
				from journal1 
				where (ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) or ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)))
				and tunnus like tcTunnus
				and parentId in (select journal.id from journal	inner join tmprekv on journal.rekvId = tmprekv.id 
...
--			raise notice 'v_account.konto %',v_account.konto; 	
--			raise notice 'v_account.asutusd %',v_account.asutusid; 	
--			raise notice 'v_saldo.db %',v_saldo.db; 	
--			raise notice 'v_saldo.kr %',v_saldo.kr; 
--			raise notice 'lnAlg  %',lnAlg ; 	
...
-- select *from tmp_subkontod_report order by asutus
	...
	if tnOpt = 4 then
		...
		for v_tunnus in 
			...
			select sum(case when ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) then (summa) else 0::numeric(18,6) end) as db, 
			sum(case when ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)) then (summa) else 0::numeric(18,6) end) as kr,
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION sp_subkontod_report(character varying, integer, date, date, integer, character varying, integer, integer)
  OWNER TO vlad;
