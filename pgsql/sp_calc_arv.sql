-- Function: sp_calc_arv_test(integer, integer, date, numeric, date)

-- DROP FUNCTION sp_calc_arv_test(integer, integer, date, numeric, date);
...
declare
	...
	v_kaart record;
	l_tulubaas_kokku numeric(14,2) = 0;
	l_tulubaas numeric(14,2) = 0;
	l_kasutatud_tulubaas numeric(14,2) = 0;
	l_period integer = 1;
	...
	l_min_sots numeric(14,2) = 0;
	v_maksud record;
	l_aasta_alg date = date(year(tdKpv),01,01);
	l_lepingu_alg_kpv date;
begin
...
if qryTooleping.algab > l_aasta_alg then
	...
	-- kontrollime kas oli vanad lepingud
	select 	algab into l_lepingu_alg_kpv 
		from tooleping 
		where parentid = qryTooleping.parentid 
		and rekvid = qryTooleping.rekvid 
		and algab < qryTooleping.algab 
		and coalesce(tooleping.lopp,date()) >  l_aasta_alg
		order by id desc limit 1;
	if l_lepingu_alg_kpv is not null and l_lepingu_alg_kpv  >  l_aasta_alg then 
		l_aasta_alg = l_lepingu_alg_kpv;
	end if;
	if l_lepingu_alg_kpv is null then
		l_aasta_alg = qryTooleping.algab ;
	end if;
--	raise notice 'qryTooleping.algab %, l_lepingu_alg_kpv %, l_aasta_alg %', qryTooleping.algab, l_lepingu_alg_kpv, l_aasta_alg;
end if;

if tnSumma is null  then -- Arvestus tehtud kasitsi 
	...
	ltSelgitus = ltSelgitus + ' Käsi arvestus '+ltEnter;
	lnSumma = tnSumma;
end if;
...
if qryPalkLib.tululiik is not null then
	--Töötuskind-lustusmakse-> tun4, Kogumispension -> tun5
	...
	if (tdPeriod is not null and tdPeriod > '1900-01-01') and year(tdPeriod) < year(tdKpv) then
		l_TKI_maar = 2;
		l_TKA_maar = 1;
		l_TM_maar = 21;
	end if;	

	raise notice 'l_TKI_maar %', l_TKI_maar;	

	lTKI = lnSumma * 0.01 * l_TKI_maar * qryPalkLib.tun4 * coalesce(case when v_kaart.tki > 0 then 1 else 0 end,0);
	
	ltSelgitus = ltSelgitus + 'TKI arvestus:' + round(lnSumma,2)::text + '*'+ (0.01 * l_TKI_maar)::text+ '*' + qryPalkLib.tun4::text + ltEnter;
	
	lPM = lnSumma * 0.01 * l_PM_maar * qryPalkLib.tun5 * coalesce(case when v_kaart.pm > 0 then 1 else 0 end,0);	
...
*/	
		lSM = lnSumma  * 0.01 * l_SM_maar * qryPalkLib.tun2 * coalesce(case when v_kaart.sm > 0 then 1 else 0 end,0);
raise notice 'lSM %, qryTooleping.minpalk %, l_SM_maar %', lSM, qryTooleping.minpalk, l_SM_maar;
		
if lSM < (qryTooleping.minpalk * (l_SM_maar * 0.01) ) and not exists (
	select id from puudumine 
		where lepingid = qryTooleping.id 
		and ( (tunnus = 1 and tyyp < 3) or (tunnus = 2 and tyyp = 1))
		and (kpv1 between date(year(tdKpv),month(tdKpv),1) and tdKpv + interval '1 month' or kpv2 between date(year(tdKpv),month(tdKpv),1) and tdKpv + interval '1 month')
		and not exists (select id from palk_oper po
			where month(po.kpv) =  month(tdKpv) and year(kpv) = year(tdKpv) 
			and po.lepingid = qryTooleping.id 
			and po.libid in (select l.id from library l inner join palk_lib pl on pl.parentid = l.id where l.rekvid =qryTooleping.rekvid and pl.liik = 5) limit 1) 
		--limit 1
		union 
		select id from palk_oper po 
			where po.lepingid in (select id from tooleping t where t.parentid = qryTooleping.parentId and t.rekvid = qryTooleping.rekvid)
			and month(po.kpv) = month(tdKpv) and year(po.kpv) = year(tdKpv)
			and po.sotsmaks is not null and po.sotsmaks > 0
		) then
		-- kui tulud puduvad, aga isiki ei ole puhkusel voi haige, siis kasutame min. sotsmaksu arvestus
	...
	raise notice '111 lSM %, qryTooleping.minpalk %', lSM, qryTooleping.minpalk;
	ltSelgitus = ltSelgitus +'Parandamine, isik ei ole puhkusel/ haigusel, kasutame min.palk  :'+ltrim(lSM::varchar) + ltEnter;
end if;	
...
-- invalidnost
	if qryTooleping.pank = 1 then
		if lnSumma >= 355 then
			ltSelgitus = ltSelgitus + 'SM parandus (puue): ' || lSM::text || ' - 117.15 = ' + (lSM - 117.15)::text + ltEnter;
			lSM = lSM - 117.15;
		else
			lSM = 0;
		end if;
	...
	lTKA = lnSumma  * 0.01 * l_TKA_maar * qryPalkLib.tun4 * coalesce(case when v_kaart.tka > 0 then 1 else 0 end,0);
--	lSM = (case when lnSumma < qryTooleping.minpalk * l_min_sots then qryTooleping.minpalk else lnSumma end) * 0.01 * l_SM_maar * qryPalkLib.tun2 * coalesce(case when v_kaart.sm > 0 then 1 else 0 end,0);

	ltSelgitus = ltSelgitus + 'TKA arvestus:' + round(lnSumma,2)::text + 
		'*' + (0.01 * l_TKA_maar)::text + '*' + qryPalkLib.tun4::text + ltEnter;

	raise notice 'qryTooleping.pohikoht %, l_aasta_alg %, lSM  -> %', qryTooleping.pohikoht, l_aasta_alg, lSM ;
	...
	if qryTooleping.pohikoht = 1 then
		...
		if year(tdKpv) >= 2015 then
			l_tulubaas_kokku = (select month(tdKpv) - month(l_aasta_alg)  + 1)  * 
				...
				raise notice 'l_aasta_alg %', l_aasta_alg;
		else
		...
		select sum(tulubaas) as tulubaas into l_kasutatud_tulubaas
				...
				and tulubaas > 0 
				and kpv >= date(year(tdKpv),01,01) and kpv <= tdKpv 
				and t.parentid = qryTooleping.parentid;

		if year(tdKpv) < 2015 then
			select sum(g31) into l_tulubaas 
				from palk_jaak 
				where lepingid = qryTooleping.id and kuu >= 1 and kuu < month(tdKpv) and aasta = year(tdKpv);
		...
		raise notice 'parandatud l_tulubaas %', l_tulubaas;
	end if;
	...
	raise notice 'lTM -> %, lnSumma ->%, l_Tulubaas -> %, lTKI %', lTM, lnSumma, l_Tulubaas, lTKI;
	if l_Tulubaas > lTM then
		l_Tulubaas = lTM;
	end if;
...
--		lTM = 0;
	...
	lTM = lTM * 0.01 * l_TM_maar;
	raise notice 'l_tulubaas %,l_tulubaas_kokku %, l_kasutatud_tulubaas %, lnSumma %, lTM %, l_TM_maar %', l_tulubaas, l_tulubaas_kokku,l_kasutatud_tulubaas, lnSumma, lTM, l_TM_maar;
...
raise notice 'l_aasta_alg %,l_kasutatud_tulubaas %,  qryTooleping.pohikoht %, lcTimestamp %, l_Tulubaas %, lSM %, lTKI %, lPM %, lTKA %, ltSelgitus %',l_aasta_alg, l_kasutatud_tulubaas, qryTooleping.pohikoht,  lcTimestamp, l_Tulubaas, lSM, lTKI, lPM, lTKA, ltSelgitus;
Return lnSumma;
...
select sp_calc_arv(138986, 290604, date(2015,08,31),null, null)

/*
select * from asutus where regkood = '35507092217'
-- 37013

select * from tooleping where parentid = 37013
-- 138153

select * from palk_kaart where lepingid = 138153 and status = 1

select * from palk_oper where lepingid in (138986,138510) and year (kpv) = 2015 and month(kpv) = 8
