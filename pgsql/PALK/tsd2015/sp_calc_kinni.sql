...
CREATE OR REPLACE FUNCTION sp_calc_kinni(
    integer,
    integer,
    date)
  RETURNS numeric AS
...
raise notice 'qryPalkLib.LIIK %, qryPalkLib.palgafond %', qryPalkLib.LIIK, qryPalkLib.palgafond;

if qryPalkLib.LIIK = 8 or (qryPalkLib.asutusest < 1 and qryPalkLib.LIIK = 7) then
	lnSumma =  sp_calc_muuda(tnlepingId, tnLibId, tdKpv);
	raise notice 'kasutame uus algoritm lnSumma %', lnSumma;
	return lnSumma;
end if;	
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION sp_calc_kinni(integer, integer, date)
  OWNER TO vlad;
...
select sp_calc_kinni(139739, 96866, date(2016,01,31))

/*
select * from asutus where regkood = '46007082211'

select * from palk_oper where lepingId in (select id from tooleping where parentId =  961 and rekvId = 63) and kpv = date(2016,01,31)
*/