...
CREATE OR REPLACE FUNCTION sp_salvesta_palk_kaart(
    integer,
    integer,
    integer,
    integer,
...
if tnId = 0 then
	...
	insert into palk_kaart (parentid,lepingid,libid,summa,percent_,tulumaks,tulumaar,status,muud,alimentid,tunnusid, minsots) 
		values (tnparentid,tnlepingid,tnlibid,tnsumma,tnpercent_,tntulumaks,tntulumaar,tnstatus,ttmuud,tnalimentid,tntunnusid, tnMinSots);
...
--end if;
  ...
  COST 100;
ALTER FUNCTION sp_salvesta_palk_kaart(integer, integer, integer, integer, numeric, integer, integer, integer, integer, text, integer, integer, character varying, numeric, integer)
  OWNER TO vlad;
