-- Function: fncarvestaviivis(integer, integer, numeric, date, integer)
/*
select * from arv where id = 490

select fncArvestaViivis1(       25,2,0.0100,date(2011,12, 2),         1)
select fncArvestaViivis1(595, 1, 0.01, date(), 1)
delete from tmp_viivis
select sum(viivis1)/15.6466 from tmp_viivis
SELECT arvnumber, summa, volg1,paev1,viivis1, tasud1, tasun1, id, tahtaeg, *  from tmp_viivis

		select leping1.asutusId, leping1.rekvId, objekt.kood as objekt  
			from leping1 left outer join library objekt on leping1.objektId = objekt.id 
			where leping1.id = 25

select fncViiviseTahtajastePaevad(492,date(), '')
select arvmuud from tmp_printarv

select fncArvestaViivis1(       25,2,0.0100,date(2011,12, 2),         1)
*/
-- DROP FUNCTION fncArvestaViivis1(490, integer, numeric, date, integer);
...
--		end if;
		...
		raise notice 'lnViiviseIntress  %',lnViiviseIntress ;
