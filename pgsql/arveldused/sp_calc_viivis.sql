...
/*
select * from asutus where nimetus like 'AVPSOFT%'
select sp_calc_viivis(1, 2, date(), 1)

*/
-- DROP FUNCTION sp_calc_viivis(integer, 2, date(), 1);
...
--					raise notice 'leitud tasud viivise arve nr  valastame paevade arv %',qryArved.number;
					...
					select arv1.muud::varchar into 	lcArveString from arv1 inner join arv on arv.id = arv1.parentid 
						where arv.rekvid = tnrekvId and arv1.muud like '%Arve nr.:'+ltrim(rtrim(qryArved.number))+'%' 
					...
					lnPaevadInString = position('Pдevad:' in lcArveString);
...
--						raise notice 'Updated 1 viivsed miinus';
			...
			for qryTasud in select kpv, (arvtasu.summa *  ifnull(dokvaluuta1.kuurs,1)) as summa 
				from arvtasu left outer join dokvaluuta1 on (dokvaluuta1.dokid = arvtasu.id and dokvaluuta1.dokliik = 22)
...
*/
--				raise notice 'viivised count %',lnCount;					
				if lnCountV > 0 	then
--					if qryarved.number = '458r8' then
						raise notice 'leitud tasud viivise arve nr  valastame paevade arv %',qryArved.number;
--					end if;
...
-- kustutame kхik kirjad kus on volg vaikesem < 50 EEK			
