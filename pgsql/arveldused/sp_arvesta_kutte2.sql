...
select * froM library where upper(kood) like upper('Kajaka 8 - 47%') and library = 'OBJEKT'
...
select sp_arvesta_kutte2(1, '',42502,  date(2011,02,28));
...
CREATE OR REPLACE FUNCTION sp_arvesta_kutte2(integer, character varying, integer, date)
...
-- ?[fncInim  * (  ?svt1 + ?svt2 + ?svt3 + ?svt4 )  * ?fncMottePaevad 
	...
	lnSumma11 = lnSumma04 * (fncteenus(lnKorterId, lnsvt1, tdKpv,0) * ifnull(lnSvt1Hind,0)+
			fncteenus(lnKorterId, lnsvt2, tdKpv,0) * ifnull(lnSvt2Hind,0) +
			fncteenus(lnKorterId, lnsvt3, tdKpv,0) * ifnull(lnSvt3Hind,0)+
			fncteenus(lnKorterId, lnsvt4, tdKpv,0)* ifnull(lnSvt4Hind,0)) * lnSumma18 / DaysInMonth(tdKpv) ;


-- ?[fncKPindA   *  ?{fncK   * ( ?mop1 + ?mop2 + ?mop3 + ?mop4 +?mop5 )

	lnSumma05 =  fnckpinda(lnKorterId, 2) * lnK *(fncteenus(lnKorterId, lnMop1, tdKpv,2) * ifnull(lnMop1Hind,0) + 
			fncteenus(lnKorterId, lnMop2, tdKpv,2) * ifnull(lnMop2Hind,0) + 
			fncteenus(lnKorterId, lnMop3, tdKpv,2) * ifnull(lnMop3Hind,0) + 
			fncteenus(lnKorterId, lnMop4, tdKpv,2) * ifnull(lnMop4Hind,0) + 
			fncteenus(lnKorterId, lnMop5, tdKpv,2) * ifnull(lnMop5Hind,0));	
...
/*
		...
		summa08, summa09, summa10, summa11,summa12, summa13, summa14, summa18)
			...
			lnSumma08, lnSumma09, lnKutteVanniHind, lnSumma11, lnSumma12, lnSumma12/lnSumma09, lnVdHind, lnSumma18 );
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION sp_arvesta_kutte2(integer, character varying,  integer, date) OWNER TO vlad;
GRANT EXECUTE ON FUNCTION sp_arvesta_kutte2(integer, character varying, integer, date) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION sp_arvesta_kutte2(integer, character varying, integer, date) TO dbpeakasutaja;
