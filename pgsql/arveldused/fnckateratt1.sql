-- Function: fnckateratt(integer, integer)

-- DROP FUNCTION fnckateratt(integer, integer);

CREATE OR REPLACE FUNCTION fnckateratt(integer, integer, varchar)
  RETURNS numeric AS
$BODY$
...
begin	
	...
	if tnUhis = 0 then
		...
		insert into tmp_fnc_selg (kpv, rekvid, timestamp, nimetus, summa01)
			select date(), 1, tcReturn, library.nimetus, nait10 
...
GRANT EXECUTE ON FUNCTION fnckateratt(integer, integer,varchar) TO dbpeakasutaja;
GRANT EXECUTE ON FUNCTION fnckateratt(integer, integer, varchar) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION fnckateratt(integer, integer, varchar) TO dbvaatleja;
