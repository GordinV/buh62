-- Function: fncveeringvool(integer, integer)

-- DROP FUNCTION fncveeringvool(integer, integer);

CREATE OR REPLACE FUNCTION fncveeringvool(integer, integer, varchar)
  RETURNS numeric AS
$BODY$
...
begin	
	...
	if tnUhis = 0 then
		...
		insert into tmp_fnc_selg (kpv, rekvid, timestamp, nimetus, summa01)
			select date(), 1, tcReturn, library.nimetus, nait11 
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION fncveeringvool(integer, integer, varchar) OWNER TO vlad;
GRANT EXECUTE ON FUNCTION fncveeringvool(integer, integer, varchar) TO dbpeakasutaja;
GRANT EXECUTE ON FUNCTION fncveeringvool(integer, integer, varchar) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION fncveeringvool(integer, integer, varchar) TO dbvaatleja;
