...
select * from arv where number = 'V9'

SELECT library.kood,dokprop.* from dokprop inner join library on library.id = dokprop.parentid where dokprop.id = 924
...
--	raise notice 'lnKontrol: %',lnKontrol;
	...
	SELECT library.kood,dokprop.* into v_dokprop from dokprop inner join library on library.id = dokprop.parentid where dokprop.id = v_arv.doklausid limit 1;
...
-- koostame selg rea
	if (select count(id) from rekv where parentid = 119 or id = 119) > 0 then
		lcSelg = '';
		for v_selg in
			select distinct nom.nimetus from arv1 inner join nomenklatuur nom on arv1.nomid = nom.id where arv1.parentid = v_arv.id
		loop
	...
	raise notice 'arv.id :%',tnId;
	raise notice 'selg :%',lcSelg;
	...
	lnJournal:= sp_salvesta_journal(0, v_arv.rekvId, v_arv.UserId, v_arv.kpv, v_arv.asutusId, ltrim(rtrim(lcselg))+' '+ltrim(rtrim(v_arv.lisa)), 
...
--	lisatud 01/02/2005
	...
	--	lisatud 18/03/2005
		...
		If v_arv.liik = 0 then	
			IF (v_arv.objektId = 1 and v_dokprop.kood <> 'ARV') then
				--	lisatud 30/12/2004 (mahakandmine), parandus 18.07.2012				
...
*/			
			...
			raise notice 'v_arv1.konto: %',v_arv1.konto;
			raise notice 'lckood5: %',lckood5;
...
*/
			...
			If v_arv1.kbm <> 0 then
			/*
				if v_dokprop.kbmkonto = '601000' then
					lcKood5 = '601';
				end if;
...
GRANT EXECUTE ON FUNCTION gen_lausend_arv(integer) TO taabel;
