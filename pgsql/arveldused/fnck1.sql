-- Function: fnck(integer, integer)

-- DROP FUNCTION fnck(integer, integer);

CREATE OR REPLACE FUNCTION fnck(integer, integer, varchar)
  RETURNS numeric AS
$BODY$
...
--		raise notice 'Parent %',lnParentObjektId;
		...
		insert into tmp_fnc_selg (kpv, rekvid, timestamp, nimetus, summa01)
			select date(), 1, tcReturn, library.nimetus, nait04
...
GRANT EXECUTE ON FUNCTION fnck(integer, integer, varchar) TO dbpeakasutaja;
GRANT EXECUTE ON FUNCTION fnck(integer, integer,varchar) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION fnck(integer, integer,varchar) TO dbvaatleja;
