-- Function: fncsoevett(integer, integer)

-- DROP FUNCTION fncsoevett(integer, integer);

CREATE OR REPLACE FUNCTION fncsoevett(integer, integer, varchar)
  RETURNS numeric AS
$BODY$
...
--
	if tnUhis = 0 then
		...
		insert into tmp_fnc_selg (kpv, rekvid, timestamp, nimetus, summa01)
			select date(), 1, tcReturn, library.nimetus, nait06
...
--		raise notice 'Parent %',lnParentObjektId;
		...
		select sum(nait06) into lnKogus
			from objekt
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION fncsoevett(integer, integer, varchar) OWNER TO vlad;
GRANT EXECUTE ON FUNCTION fncsoevett(integer, integer, varchar) TO dbpeakasutaja;
GRANT EXECUTE ON FUNCTION fncsoevett(integer, integer, varchar) TO dbkasutaja;
GRANT EXECUTE ON FUNCTION fncsoevett(integer, integer, varchar) TO dbvaatleja;
