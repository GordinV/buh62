...
if tnSvod = 1 then
	...
	if tnOpt = 1 then
		...
		-- kaibed
			select sum(case when ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) then (summa*ifnull(dokvaluuta1.kuurs,1)) else 0::numeric(18,6) end) as db, 
				sum(case when ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)) then (summa*ifnull(dokvaluuta1.kuurs,1)) else 0::numeric(18,6) end) as kr into v_kaibed 
			...
			select sum(case when ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) then (summa*ifnull(dokvaluuta1.kuurs,1)) else 0::numeric(18,6)  end) as db, 
				sum(case when ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)) then (summa*ifnull(dokvaluuta1.kuurs,1)) else 0::numeric(18,6) end ) as kr into v_saldo
				from journal1  left outer join dokvaluuta1 on (dokvaluuta1.dokid = journal1.id and dokvaluuta1.dokliik = 1)
				where (ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) or ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)))
				and tunnus like tcTunnus
				and parentId in (select journal.id from journal	inner join tmprekv on journal.rekvId = tmprekv.id 
...
-- select *from tmp_subkontod_report order by asutus
	...
	if tnOpt = 4 then
		...
		-- kaibeasutus andmik tunnuse jдrgi
		...
		for v_tunnus in 
			...
			select sum(case when ltrim(rtrim(deebet)) = ltrim(rtrim(v_account.konto)) then (summa*ifnull(dokvaluuta1.kuurs,1)) else 0::numeric(18,6) end) as db, 
			sum(case when ltrim(rtrim(kreedit)) = ltrim(rtrim(v_account.konto)) then (summa*ifnull(dokvaluuta1.kuurs,1)) else 0::numeric(18,6) end) as kr,
