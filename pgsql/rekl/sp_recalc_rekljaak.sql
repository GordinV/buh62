...
if v_luba.staatus > 0 then
	...
	select sum(summa * ifnull(dokvaluuta1.kuurs,1)) into lnKr1 from toiming left outer join dokvaluuta1 on (toiming.id = dokvaluuta1.dokid and dokliik = 24)
		where lubaId = tnId 
		and ((tyyp IN ('DEKL') and not empty (saadetud)) OR tyyp in ('PARANDUS'))
	...
	select sum(summa * ifnull(dokvaluuta1.kuurs,1) ) into lnKr2 from toiming left outer join dokvaluuta1 on (toiming.id = dokvaluuta1.dokid and dokliik = 24)
		where lubaId = tnId 
		and ((tyyp IN ('DEKL') and not empty (saadetud)) OR tyyp in ('PARANDUS','INTRESS'))
		and staatus > 0 
		and tahtaeg <= date();
	--and staatus = 1
	...
	*/
	...
	raise notice 'alg %,jaak %, volg %, tasu % ',lnAlgsaldo, lnKr1, lnKr2, lnDb;
...
ALTER FUNCTION sp_recalc_rekljaak(integer)
  OWNER TO vlad;
GRANT EXECUTE ON FUNCTION sp_recalc_rekljaak(integer) TO vlad;
GRANT EXECUTE ON FUNCTION sp_recalc_rekljaak(integer) TO public;
