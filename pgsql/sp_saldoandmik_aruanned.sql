...
--				update tmp_saldoandmik set timestamp = lcRetElim where ltrim(rtrim(timestamp)) = ltrim(rtrim(lcreturn)) and left(konto,1) = '7';	
				...
				delete from tmp_saldoandmik where ltrim(rtrim(timestamp)) = ltrim(rtrim(lcreturn)) and left(ltrim(rtrim(konto)),1) = '7';
...
GRANT EXECUTE ON FUNCTION sp_saldoandmik_aruanned(integer, date, integer, integer, integer, integer) TO dbkasutaja;


select sp_saldoandmik_aruanned(63, date(2017,09,30), 2017, 2, 1, 0);

select * from tmp_sk_aruanned 
order by id desc 
limit 10
where timestamp = '63201709142734045'
