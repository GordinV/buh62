...
select sp_vanemtasu_aruanne2(         9,'%%','%%','%Ilja Olhovetski%', DATE(2011, 1, 1), DATE(2011, 1,31),'%%')
...
--	ldPrevKpv = date(year(ldPrevKpv),month(ldPrevKpv),1);
	...
	raise notice ' v_arved.dokkpv%',v_arved.dokkpv ;
	...
	--	tasu
		...
		ldKpv = date(year(tdKpv1-1),month(tdKpv1-1),1);
		...
		raise notice ' v_tasud.summa %',v_tasud.summa;
		raise notice ' v_arved.isikid%',v_arved.isikid;
		raise notice 'v_arved.tunnus %',v_arved.tunnus;
		raise notice 'tdKpv1 %',tdKpv1;
