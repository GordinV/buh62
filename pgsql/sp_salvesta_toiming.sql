-- Function: sp_salvesta_toiming(integer, integer, integer, date, character varying, character varying, date, numeric, integer, character varying, text, integer, integer, integer)

-- DROP FUNCTION sp_salvesta_toiming(integer, integer, integer, date, character varying, character varying, date, numeric, integer, character varying, text, integer, integer, integer);
...
if tnId = 0 then
	...
	-- uus kiri
		insert into toiming (parentid,lubaid, kpv,alus, ettekirjutus, tahtaeg, summa, staatus, tyyp, muud, failid, dokpropId, number) 
			values (lnparentid,tnlubaid, tdkpv, tcalus, tcettekirjutus, tdtahtaeg, round(tnsumma,0), tnstaatus, tctyyp, ttmuud, tnFailid, tnDokpropId, tnNumber);
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION sp_salvesta_toiming(integer, integer, integer, date, character varying, character varying, date, numeric, integer, character varying, text, integer, integer, integer, character varying, numeric) OWNER TO vlad;
