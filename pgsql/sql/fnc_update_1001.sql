...
select sp_update_palk_jaak(date(2010,01,01), date(2010,01,31), 1, 3);
*/
...
--	select Tulubaas into lnTulubaas from palk_config where rekvid = tnRekvId;
	...
	select palk_config.tulubaas * ifnull(dokvaluuta1.kuurs,1) into lnTulubaas 
		from palk_config left outer join dokvaluuta1 on (palk_config.id = dokvaluuta1.dokliik = 26) where palk_config.rekvid = tnrekvId;
...
select palk_config.*, ifnull(dokvaluuta1.valuuta,'EEK')::varchar as valuuta, ifnull(dokvaluuta1.kuurs,1)::numeric as kuurs into v_palk_config 
	from palk_config left outer join dokvaluuta1 on (palk_config.id = dokvaluuta1.dokliik = 26) where palk_config.rekvid = lnrekv;
...
select palk_config.*, ifnull(dokvaluuta1.valuuta,'EEK')::varchar as valuuta, ifnull(dokvaluuta1.kuurs,1)::numeric as kuurs into v_palk_config 
	from palk_config left outer join dokvaluuta1 on (palk_config.id = dokvaluuta1.dokliik = 26) where palk_config.rekvid = qryTooleping.rekvid;
--select rekvId into lnrekv from library where id = qryPalkLib.parentId;
