...
public gnHandle,gnHandleAsync, gcWindow, gRekv,gUserId, oPrintform2, oFinder, gReturn, oAsutused, oUserid,;
	oArved, oDokLausend, oNomenklatuur, oDok, oTasud,;
	oAsutused, oUserid, oRekv, 	cKasutaja, oDb, oPeriod,gnKuu, gnAasta, gdKpv, gVersia, gcDatabase, glError, oRI, oConnect, ;
	oMenu, gcKey, gcAmetnik, gcPrognimi, oEeltaotlus, oEelVariandid, oEelarve, oTuludeAllikad, oTulutaitm, oKuluTaitm, oetsd
...
If  .Not. Empty(config.reServed1) .And. File('updater1.exe')
	lrEsult = chEckuuendused('www.avpsoft.ee/downloads/eelproj/uuendus.dbf')
	If Used('ajalugu')
		Use In ajAlugu
	Endif
	If Used('uuendus')
