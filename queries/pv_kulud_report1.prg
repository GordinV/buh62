...
TEXT TO l_params TEXTMERGE noshow
	<<IIF(!EMPTY(fltrAruanne.kood4),'Konto=','')>> <<ALLTRIM(fltrAruanne.konto)>> <<IIF(!EMPTY(fltrAruanne.konto),'%','')>> 
ENDTEXT
...
IF !ISNULL(fltrAruanne.asutusid) AND !EMPTY(fltrAruanne.asutusid) 
...
lError = oDb.readFromModel('aruanned\raamatupidamine\pv_kulud', 'pv_kulud_report', 'fltrAruanne.kpv1,fltrAruanne.kpv2,gRekv,l_params', 'pv_kulud_report1', l_where )
