...
lError=doktyyp_kontrollimine('VORDER','Kassa väljaminelu orderid')
...
If used ('v_library') and reccount ('v_library') > 0
	...
	With oDb
		...
		If lError = .f.
			.rollback()
			If config.debug = 1
				Messagebox ('Viga, esimine statr ebaõnnestus','Kontrol')
			Endif
		Else
			.commit()
		Endif
...
Function tehe_pv_profill
	Wait window [PV hälestamine...] nowait
	With oDb
&& PV gruppid
		...
		If reccount ('comGrupp') < 1
			...
			Insert into v_library (rekvid, library,kood, nimetus) values;
				(gRekv, 'PVGRUPP','ARVUTID','Arvutid ja büro tehhnika')
			Insert into v_library (rekvid, library,kood, nimetus) values;
...
&& PV Operatsioonid
		...
		If !found ()
			...
			Endif
			Insert into v_nomenklatuur (rekvid, kood, nimetus, dok) values ;
				(gRekv,'PAIGUTUS', 'PV paigutamine', 'PAIGUTUS')
		Endif
		Select comNomremote
		...
		If !found ()
			...
			Endif
			Insert into v_nomenklatuur (rekvid, kood, nimetus, dok) values ;
				(grekv, 'KULUM', 'Kulumi arvestamine', 'KULUM')
		Endif
		Select comNomremote
		...
		If !found ()
			...
			Endif
			Insert into v_nomenklatuur (rekvid, kood, nimetus, dok) values ;
				(gRekv, 'MAHAKANDMINE', 'PV likvideerimine', 'MAHAKANDMINE')
		Endif
		Select comNomremote
		...
		If !found ()
			...
			Endif
			Insert into v_nomenklatuur (rekvid, kood, nimetus, dok) values ;
				(grekv,'PARANDUS', 'PV parandamine', 'PARANDUS')
		Endif
...
Function tehe_palga_profill
	Wait window [Palga hälestamine...] nowait
	With oDb
		If !used ('v_library')
			.use ('v_library','v_library',.t.)
		Endif
		...
		If lError = .t.
			...
			Zap
			Insert into v_library (rekvid, library,kood, nimetus) values;
				(gRekv, 'PALK','TMI','Töökindlustusmaks isikutelt 1%')
			lError = .cursorupdate('v_library')
			If lError = .t.
				Insert into v_palk_lib (parentid,liik,tund,maks, palgafond, asutusest, algoritm,round) values ;
		...
		If lError = .t.
			...
			Zap
			Insert into v_library (rekvid, library,kood, nimetus) values;
				(gRekv, 'PALK','TMA','Töökindlustusmaks asutuselt 0.5%')
			lError = .cursorupdate('v_library')
			If lError = .t.
				Insert into v_palk_lib (parentid,liik,tund,maks, palgafond, asutusest, algoritm,round) values ;
