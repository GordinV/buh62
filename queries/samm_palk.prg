...
*!*			messagebox (iif(config.keel = 1,'Ошибка: недопустимая комбинация классификаторов',;
*!*				'Viga: ebaхigus klassifikaatori kombinatsioon'),'Kontrol')
*!*			return .f.
*!*		endif
*!*	endif
...
Procedure get_osakonna_list
	...
	If reccount('curvalitud') > 0
		Zap
	Endif
	Do form forms\samm with '1', iif(config.keel = 2,'Osakonnad','Отделы'),iif(config.keel = 2,;
		'Valitud osakonnad','Выбранные отделы') to nResult
	If nResult = 1
...
Procedure get_isiku_list
	...
	If reccount('curvalitud') > 0
		Zap
	Endif
	Do form forms\samm with '2', iif(config.keel = 2,'Isikud','Работники'),;
		iif(config.keel = 2,'Valitud isikud','Выбранные работники') to nResult
	If nResult = 1
		Select distinc id from curValitud into cursor query1
		Select query1
		Scan
...
Procedure get_kood_list
	...
	If reccount('curvalitud') > 0
		Zap
	Endif
	Do form forms\samm with '3', iif(config.keel = 2,'Palgastruktuur','Начисления и удержания'),iif(config.keel = 2,;
		'Valitud ','Выбранно ') to nResult
	If nResult = 1
		Select distinc id from curValitud  into cursor query1
		Select query1
		Scan
