...
select sum(sotsmaks) as sotsmaks, sum(tulumaks) as tulumaks, sum(palk) as palk, sum(tm) as tm,;
	sum(atm) as atm, sum(palksots) as palksots, sum(pm) as pm from printtsd into cursor printtsd_
...
Create Cursor ftsd_report (asutus c(254) Default qryRekv.nimetus, regnum c(20) Default qryRekv.regkood,;
	...
	k_sots Y, k_tulu Y, k_palk Y, k_tm Y, k_atm Y, k_pm Y, k_arvsots Y, g31 Y, g32 N(18,6), g71 c(20), g711 y, g72 c(20), g721 y,;
...
*!*			if printtsd1.isikukood = '47104013758'
*!*				set step on
*!*			endif
...
*!*			Endif
		...
		If !Empty(lc71) And Empty(lc72) And (printtsd1.palk_02+printtsd1.palk_03+printtsd1.palk_04+printtsd1.palk_05+;
			...
			Endif
			If Empty(lc72) And lc71 <> '22' And printtsd1.palk_22 <> 0
				lc72 = '22'
		...
		Replace k_sots With ROUND(printTsd.SOTSMAKS/fltrPrinter.kuurs,2) ,;
			...
			k_arvsots With ROUND(printTsd.palk/fltrPrinter.kuurs,2),;
	...
	If lnAddrec > 0
		For i = 1 To lnAddrec
			Append Blank
			Replace k_sots With ROUND(printTsd_.SOTSMAKS/fltrPrinter.kuurs,2) ,;
				k_tulu With ROUND(printTsd_.TULUMAKS/fltrPrinter.kuurs,2),;
				k_palk With ROUND(printTsd_.palk/fltrPrinter.kuurs,2),;
				k_arvsots With ROUND(printTsd_.palksots/fltrPrinter.kuurs,2),;
				k_atm With ROUND(lnatm/fltrPrinter.kuurs,2),;
				k_tm With ROUND(printTsd_.tm/fltrPrinter.kuurs,2),;
				k_pm With ROUND(printTsd_.pm/fltrPrinter.kuurs,2) In ftsd_report
		Endfor
	Endif
	Use In printTsd
	Use In printtsd1
...
IF !USED('comTululiik')
	...
	INSERT INTO comTululiik (kood, nimetus) VALUES ('22','Töötaja III sambasse tehtud sissemakse')
ENDIF
SELECT comTululiik
