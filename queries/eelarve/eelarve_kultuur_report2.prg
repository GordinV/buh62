...
*set step on
	...
	*		wait window 'Serveri poolt funktsioon ...' nowait
		...
		If Used('qryEelarve1')
			...
			lcString = "select konto, tegev, eelarve, allikas, rahavoo, sum(db) as db, sum(kr) as kr, AllAsutus, RekvIdSub from tmp_eelarve_aruanne1 where rekvid = "+str(gRekv)+;
			" and timestamp = '"+tcTimestamp +"' group by konto, tegev, allikas, rahavoo, eelarve, AllAsutus, RekvIdSub"+;
			" order by AllAsutus,konto, allikas, tegev, eelarve "
