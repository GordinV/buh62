...
Select allikas, tegev, artikkel, nimetus, tunnus, proj, uritus, objekt,;
	sum(eelarve) as eelarve, sum(eelarve_kassa) as eelarve_kassa, sum(taitmine) as taitmine, sum(taitmine_kassa) as taitmine_kassa,;
	asutus ;
	from tmpReport ;
	GROUP By rekv_id , asutus, allikas, tegev, artikkel, nimetus, tunnus, proj, uritus, objekt;
	ORDER By rekv_id desc, artikkel, allikas, tegev, tunnus, proj, uritus, objekt ;
