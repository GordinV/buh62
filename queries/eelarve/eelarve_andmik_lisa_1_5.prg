...
SELECT 'S' as is_esita, ;
	...
	sum(tegelik) as tegelik, sum(kassa) as kassa, sum(saldoandmik) as saldoandmik ;
	FROM qryReport ;
	where (artikkel  in ('382','3818') OR LEFT(artikkel,3) in ('388'));
	INTO CURSOR tmp1
...
SELECT 'S' as is_esita, ;
	...
	(q.eelarve_kassa) - ((tmp_tulud_kokku.eelarve_kassa + tmp_kulud_kokku.eelarve_kassa) + investeerimis_tegevus.eelarve_kassa) + finants_tegenus.eelarve_kassa as eelarve_kassa, ;
	(q.eelarve_taps) - ((tmp_tulud_kokku.eelarve_taps + tmp_kulud_kokku.eelarve_taps) + investeerimis_tegevus.eelarve_taps) + ;
	finants_tegenus.eelarve_taps as eelarve_taps, ;
	(q.eelarve_kassa_taps) - ((tmp_tulud_kokku.eelarve_kassa_taps + tmp_kulud_kokku.eelarve_kassa_taps) + ;
