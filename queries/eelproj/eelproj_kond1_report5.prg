...
SET STEP ON 
create cursor eelarve_report1 (rekvid int, subrekvid int, asutus c(254), subasutus c(254), allikas c(20), tegev c(20), eelarve c(20),; 
	nimetus c(254), summa1 n(18,6), summa2 n(18,6), summa3 n(18,6), summa4 n(18,6), summa5 n(18,6), summa6 n(18,6),;
	summa11 n(18,6), summa12 n(18,6), summa13 n(18,6), summa14 n(18,6), summa15 n(18,6), summa16 n(18,6))
	...
	*		wait window 'Serveri poolt funktsioon ...' nowait
		...
		lError = oDb.Exec("sp_eelarve_aruanne1 ", Str(grekv)+;
			...
			ltrim(rtrim(fltrAruanne.proj))+"',6,"+;
			Str(fltrAruanne.kond,9),"qryEelarve1")

		If Used('qryEelarve1')
			tcTimestamp = Alltrim(qryEelarve1.sp_eelarve_aruanne1)
			lcString = "select RekvIdSub as rekvid, rekv.nimetus as asutus,tmp_eelproj_aruanne1.allikas, tmp_eelproj_aruanne1.tegev,"+;
			" tmp_eelproj_aruanne1.Eelarve, tmp_eelproj_aruanne1.Nimetus, "+;
			" summa1, summa2, summa3, summa4, summa5, summa11, summa12, summa13, summa14, summa15, summa1 + summa2 + "+;
			" summa3 + summa4 + summa5 as summa6 "+;
			"  from tmp_eelproj_aruanne1 inner join rekv on tmp_eelproj_aruanne1.RekvIdSub = rekv.id  where rekvid = "+str(gRekv)+;
			" and timestamp = '"+tcTimestamp +"' order by RekvIdSub,allikas, tegev, eelarve"
			lError = oDb.Execsql(lcString, 'eelarve_report_tmp')
			...
			scan
				...
				insert into eelarve_report1 (rekvid, subrekvid, asutus, subasutus,allikas, tegev, eelarve,	nimetus, ;
					summa1, summa2, summa3, summa4, summa5, summa11, summa12, summa13, summa14, summa15,summa6);
					values (lnrekvid, lnRekvidSub, lcRekvNimi, lcRekvSubNimi, eelarve_report_tmp.allikas, eelarve_report_tmp.tegev,;
					 eelarve_report_tmp.eelarve,	eelarve_report_tmp.nimetus, ;
					eelarve_report_tmp.summa1, eelarve_report_tmp.summa2, eelarve_report_tmp.summa3, eelarve_report_tmp.summa4,;
					eelarve_report_tmp.summa5, eelarve_report_tmp.summa11, eelarve_report_tmp.summa12, eelarve_report_tmp.summa13, ;
					eelarve_report_tmp.summa14,	eelarve_report_tmp.summa15, eelarve_report_tmp.summa6)
