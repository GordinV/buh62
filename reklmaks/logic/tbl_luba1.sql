-- Function: sp_calc_kinni(int4, int4, date)

-- DROP FUNCTION sp_calc_kinni(int4, int4, date);
--muudetud 04/01/2005
CREATE OR REPLACE FUNCTION sp_calc_kinni(int4, int4, date)
  RETURNS "numeric" AS
'
declare
...
-- muudetud 03/01/2005
IF lnSumma > 0 AND (qryPalkLib.liik = 7 OR qryPalkLib.liik = 8) then
	
	-- kontrol kas on maksed avansimaksetest
	select SUM(summa) inTO lnAvans from palk_oper 
		where lepingId = tnLepingId AND YEAR(kpv) = YEAR(tdKpv) and MONTH(kpv) = MONTH(tdKpv)  AND libId = tnLibId 
			AND MUUD = ''AVANS'';
		IF lnAvans > 0 then
			lnSumma := lnSumma - lnAvans;
		END IF;
END IF;
...
GRANT EXECUTE ON FUNCTION sp_calc_kinni(int4, int4, date) TO public;
GRANT EXECUTE ON FUNCTION sp_calc_kinni(int4, int4, date) TO GROUP dbkasutaja;
