-- Function: sp_calc_intress(integer, date)

-- DROP FUNCTION sp_calc_intress(integer, date);
...
begin
	-- kustuma vana arvestus

	delete from viiviseinfo  where intressid in (select id from toiming where tyyp = 'INTRESS' and lubaid = tnLubaId and kpv = tdKpv and staatus = 0);	
	delete from toiming where tyyp = 'INTRESS' and lubaid = tnLubaId and kpv = tdKpv and staatus = 0;
...
-- koostame deklaratsioonide nimekiri, kus on staatus < 3, tahtaeg < tdKpv and tyyp = 'DEKL'	
	...
	loop
		raise notice 'v_dekl.id %',v_dekl.id;
		lnpaev = 0;
		lnSumma0 = 0;
		lnSumma1 = 0;
		lnSumma2 = 0;
