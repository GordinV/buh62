...
public gnHandle,gnHandleAsync, gcWindow, gRekv,gUserId, oPrintform2, oFinder, gReturn, oAsutused, oUserid,;
	oDokLausend, oNomenklatuur, oSorder, oVorder, oKontod, oAllikad, oArtikkel, oTegev,;
	oObjekt, oTunnus,oJournal, oSaldo, oAsutused, oUserid, oRekv, oLausendid, oKlsaldo, oDok, ;
	cKasutaja, oDb, oOsakonnad, oAmetid, oPalklib, oPalkOper, oTaabel1, oPeriod,;
	oHoliday, gnKuu, gnAasta, gdKpv, oPalkJaak, ;
...
cFile = 'PALK\MENUBAR.DBF'
if file (cFile)
	use (cFile) in 0 alias menubar
else
	use menubar in 0 
endif
cFile = 'PALK\MENUITEM.DBF'
if file (cFile)
...
set classlib to toolspalk additive
oTools = createobject('Toolspalk')
oTools.translate()
oTools.show()
if config.debug = 0
	on error do err
