...
Do Case
	Case gversia = 'VFP'
		...
		Scan For Mline(qryKey.Connect,1) = 'FOX'
			...
			WAIT WINDOW 'Parandan andmebaas:' + lcData TIMEOUT 1 
			lcdataproc = lcdefault+'\tmp\buhdata5_20060830.sql'
	
			If File (lcdata) AND FILE(lcdataproc)
...
*				lError =  _alter_vfp()
				Close Data
				WAIT WINDOW 'Parandan andmebaas: done' TIMEOUT 1 
			ELSE
				WAIT WINDOW 'Andmebaasi parandamine ebaхnnestus: failid ei leidnud' TIMEOUT 1 
...
Function update_curprinter
	...
	Select curPrinter
	Locate For alltrim(objekt) = 'PvAruanne'And Id = 93
