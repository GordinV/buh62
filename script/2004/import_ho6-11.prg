...
lcFile1 = 'c:\avpsoft\haridus\fakt\fakt11.dbf'
...
*!*	* otsime asutusId
*!*		Select asutused
*!*		Locate For Val(Alltrim(n1)) = qryHO.tarnija_ko
*!*		If Found()


*!*			lcregkood = Ltrim(Rtrim(asutused.n6))
...
*!*			lcNimi = Ltrim(Rtrim(qryHO.tarnija_ni))
*!*	*!*	*!*		If lcregkood = '100004252'
*!*	*!*	*!*			lcregkood = '10004252'
*!*	*!*	*!*		Endif

*!*			Do Case
*!*				Case qryHO.tarnija_ko = 925
*!*					lcNimi = 'Longest Trade OU'

*!*				Case qryHO.tarnija_ko = 415
*!*					lcNimi = 'Benavento OU'

*!*				Case qryHO.tarnija_ko = 416
*!*					lcNimi = 'Step Systems OU'
*!*			Endcase

*!*			lcString = "select id from asutus where UPPER(LTRIM(RTRIM(nimetus))) = '"+UPPER(lcNimi)+"'"
*!*			lnresult = SQLEXEC(gnHandle,lcString, 'qryId')
*!*			If Reccount('qryid') <  1
*!*				lcString = " insert into asutus (regkood, nimetus, rekvId, tp) values ( "+;
*!*					"' ','"+Ltrim(Rtrim(lcNimi))+"',11,'"+;
*!*					LEFT(Ltrim(Rtrim(Str(qryHO.tp_d_t_))),6)+"')"
...
* otsime asutusId
	...
	lcSelg = Ltrim(Rtrim(qryHo.text))
	lcDok = Ltrim(Rtrim((qryHO.mo)))
	lcMuud = ''
	lcUksus = ''

	lcDeebet = Left(Ltrim(Rtrim(Str(qryHO.d_t_konto))),6)
...
*	lcKreedit = Iif(lcKreedit = '382080','382090',lcKreedit)
	...
	lcTpd = Ltrim(Rtrim(Str(qryHO.tp_d_t_)))
...
*!*		lcKood5 = Iif('352.00.17' $ lcKood5,'352.00',lcKood5)
	lcTunnus = Alltrim(Str(qryHO.asutus))
