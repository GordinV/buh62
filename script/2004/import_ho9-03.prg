...
* maks 08 kuu eest
			...
			If lnresult > 0
				...
				lcString = "insert into vanemtasu3 (rekvid, userid ,  opt ,  kpv ,  tunnus ,  muud ) values ("+;
					" 31, 222, 1, DATE(2004,07,31), '"+Ltrim(Rtrim(qryHO.kood))+"','tasud 07/2004 kuu eest')"
				lnresult =SQLEXEC(gnHandle,lcString)
				If lnresult > 0
					lcString = " SELECT id FROM vanemtasu3 ORDER BY id desc limit 1"
					lnresult =SQLEXEC(gnHandle,lcString,'qryDokId')

					If !Empty(qryHO.s2+qryHO.s2a+qryHO.s2u) And lnresult > 0
						lcString = " insert into vanemtasu4 (parentid,isikid , maksjakood , maksjanimi ,  nomid ,  kogus ,  hind ,  summa ) values ("+;
							...
							LTRIM(Rtrim(qryIsikId.vanemnimi))+"',0,1,"+Str(qryHO.s2+qryHO.s2a+qryHO.s2u,12,2)+","+Str(qryHO.s2+qryHO.s2a+qryHO.s2u,12,2)+")"
						lnresult =SQLEXEC(gnHandle,lcString)
					Endif
				Endif
		...
		If lnresult < 1
			...
			Exit
		Endif
		lnSumma = lnSumma + qryHO.s2+qryHO.s2a+qryHO.s2u
		lnrec = lnrec + 1
	Endscan
