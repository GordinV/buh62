...
lcString = 'delete from eelarve where rekvid = 31'
...
If lnResult > 0
	...
	Scan
		...
		lcString = "select id from library where rekvid = 31 and library = 'TUNNUS' and kood = '"+LTRIM(RTRIM(qryHO.n_ksus))+"'"
		...
		If !Empty(qryHO.allikas)
			lcString = "select id from library where library = 'ALLIKAD' and kood = '"+LTRIM(RTRIM(Str(qryHO.allikas)))+"'"
			lnResult = SQLEXEC(gnHandle,lcString,'qryId')
			If lnResult < 1 Or Reccount('qryId') < 1
				lnResult = -1
				Set Step On
				Exit
			Else
				lcKood2 = LTRIM(RTRIM(Str(qryHO.allikas)))
			Endif
		Endif
		...
		lcString = "select id from library where library = 'TULUDEALLIKAD' and kood = '"+qryHO.tulukood+"'"
		lnResult = SQLEXEC(gnHandle,lcString,'qryId')
		If lnResult < 1 Or Reccount('qryId') < 1
			lnResult = -1
			Set Step On
		...
		If lnResult > 0
			lcString = "insert into eelarve (aasta, rekvid, tunnusId, kood1, kood2, kood4, summa, muud) values (2004,31,"+;
				STR(lnTunnusId,31)+",'"+lcKood1+"','"+lcKood2+"','"+lcKood4+"',"+(Alltrim(qryHO.Summa))+",'"+qryHO.m_rkused+"')"
			lnResult = SQLEXEC(gnHandle,lcString)
		Endif
		...
		If lnResult < 1
			...
			Exit
		Endif
		lnSumma = lnSumma + Val((Alltrim(qryHO.Summa)))
		lnrec = lnrec + 1
	Endscan
