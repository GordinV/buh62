...
lcFile1 = 'c:\avpsoft\haridus\alg\alg.dbf'
lcFile2 = 'c:\avpsoft\haridus\alg\asu.dbf'
If !File(lcFile1)
	Messagebox('Viga, failid ei leidnud')
	lError = .T.
Endif
Use (lcFile1) In 0 Alias qryHO
Use (lcFile2) In 0 Alias qryAsu
...
SELECT n2 as konto, sum(VAL(transdigit(ALLTRIM(n3)))) as summa FROM qryHo  ;
WHERE VAL(ALLTRIM(n2)) > 0 ;
group by n2;
INTO CURSOR qryKonto
...
Select qryKonto
Scan
	Wait Window qrykonto.konto+Str(Recno('qryKonto'))+'-'+Str(Reccount('qryKonto')) Nowait
	lcErr = ''
	SELECT qryHo 
	SELECT qryAsu.n6 as regkood FROM qryAsu WHERE n1 in (select n4 FROM qryHo WHERE n2 = qryKonto.konto) INTO CURSOR qrySub
	SELECT qrySub 
	BROWSE NOWAIT 

* otsime asutusId
*!*		lcregkood = Ltrim(Rtrim(qryHO.reg_kood))
*!*		If lcregkood = '100004252'
*!*			lcregkood = '10004252'
*!*		Endif

	lcString = "select id from library where library = 'KONTOD' and kood = '"+qryKonto.konto+"'"
...
*!*			Endif
	...
	lcDok = Ltrim(Rtrim(qryHO.n_________))
	lcMuud = ''
	lcUksus = ''
	...
	lcDeebet = Iif(lcDeebet = '200201','202001',lcDeebet)
...
*	lcKreedit = Iif(lcKreedit = '382080','382090',lcKreedit)
	...
	lcKreedit = Iif(lcKreedit = '200201','202001',lcKreedit)


	lcTpd = Ltrim(Rtrim(Str(qryHO.tp_d_t)))
	lcTpk = Ltrim(Rtrim(qryHO.tp_k_t))
...
*	transdigit(Ltrim(Rtrim(qryHO.n9)))
	...
	lcTunnus = Alltrim(STR(qryHO.ASUTUS_KOO,8))
