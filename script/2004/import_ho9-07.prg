...
lcFile1 = 'c:\avpsoft\haridus\lapsed\lapsed08_a.dbf'
...
* puudub
				...
				lcString = " insert into vanemtasu1 ( isikkood, nimi,  vanemkood ,  vanemnimi ,  muud ) values ('"+;
					lcisikkood+"','"+lcNimi+"','"+str(qryHO.isikr,11)+"','"+Ltrim(Rtrim(qryHO.nimr))+"','"+;
...
* arv
		If lnresult > 0
			If !Empty(qryHO.arv+qryHO.arva+qryHO.arvu) And lnresult > 0
				lcDate = "date(2004,08,31)"
				lcString = "insert into vanemtasu3 (rekvid, userid ,  opt ,  kpv ,  tunnus ,  muud ) values ("+;
					" 31, 222, 2,"+lcdate +", '"+Ltrim(Rtrim(qryHO.kood))+"','imp. fail ddu08_vlad')"
				lnresult =SQLEXEC(gnHandle,lcString)
				If lnresult > 0
			...
			If qryHO.arv<> 0
				If lnresult > 0
						lcString = " insert into vanemtasu4 (parentid,isikid , maksjakood , maksjanimi ,  nomid ,  kogus ,  hind ,  summa ) values ("+;
							...
							LTRIM(Rtrim(qryIsikId.vanemnimi))+"',852,1,"+Str(qryHO.arv,12,2)+","+Str(qryHO.arv,12,2)+")"
						lnresult =SQLEXEC(gnHandle,lcString)
			...
			If qryHO.arvA<> 0
				If lnresult > 0
						lcString = " insert into vanemtasu4 (parentid,isikid , maksjakood , maksjanimi ,  nomid ,  kogus ,  hind ,  summa ) values ("+;
							STR(qryDokId.Id,9)+","+Str(qryIsikId.Id,9)+",'"+Ltrim(Rtrim(qryIsikId.vanemkood))+"','"+;
							LTRIM(Rtrim(qryIsikId.vanemnimi))+"',853,1,"+Str(qryHO.arva,12,2)+","+Str(qryHO.arva,12,2)+")"
						lnresult =SQLEXEC(gnHandle,lcString)
			...
			If qryHO.arvu<> 0
				If lnresult > 0
						lcString = " insert into vanemtasu4 (parentid,isikid , maksjakood , maksjanimi ,  nomid ,  kogus ,  hind ,  summa ) values ("+;
							STR(qryDokId.Id,9)+","+Str(qryIsikId.Id,9)+",'"+Ltrim(Rtrim(qryIsikId.vanemkood))+"','"+;
							LTRIM(Rtrim(qryIsikId.vanemnimi))+"',855,1,"+Str(qryHO.arvu,12,2)+","+Str(qryHO.arvu,12,2)+")"
						lnresult =SQLEXEC(gnHandle,lcString)
		...
		If lnresult < 1
			...
			Exit
		Endif
		lnSumma = lnSumma + qryHO.arv+qryHO.arva+qryHO.arvu
		lnrec = lnrec + 1
	Endscan
