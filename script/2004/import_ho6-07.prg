...
lcFile1 = 'c:\avpsoft\haridus\fakt\fakt8.dbf'
...
*!*			Endif
	...
	lcAasta = STR(year(qryHo.data),4)
	lcKuu = STR(month(qryHo.data),2)
	lcPaev = str(day(qryHo.data),2)
	...
	lcKreedit = Left(Ltrim(Rtrim((qryHO.k_t_konto))),6)
...
*	lcKreedit = Iif(lcKreedit = '382080','382090',lcKreedit)
	...
	lcTpd = Ltrim(Rtrim((qryHO.tp_d_t)))
	lcTpk = Ltrim(Rtrim((qryHO.tp_k_t)))
	lcSumma = str(qryHO.summ,14,2)
*	transdigit(Ltrim(Rtrim(qryHO.n9)))
	lcKood1 = Ltrim(Rtrim(qryHO.tt))
	lcKood2 = Ltrim(Rtrim(STR(qryHO.alikas,8)))
	lcKood3 = ''
	lcKood4 = ''
	lcKood5 = Ltrim(Rtrim(str(qryHO.eelarve_va,8)))
	...
	lcTunnus = Alltrim(str(qryHO.kood_asutu,9))
