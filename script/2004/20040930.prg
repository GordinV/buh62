...
Function _alter_vfp
	...
	lnElement = Ascan(aObj,Upper('TUNNUSID'))
	If lnElement < 1
		Alter Table leping1 Add Column tunnusId Int Default 0
	Endif
	...
	lnElement = Ascan(aObj,Upper('TUNNUSID'))
	If lnElement < 1
		Alter Table leping3 Add Column tunnusId Int Default 0
	Endif
...
*!*			CREATE TRIGGER ON vanemtasu4   FOR DELETE  AS trigd_vanemtasu4()
	...
	Wait Window 'Stored procedure: sp_subkontod_report' Nowait


	lcFile = 'tmp/sp_subkontod_report.sql'
	If File(lcFile)


		If Empty(CHECK_obj_pg('PROC',UPPER('sp_subkontod_report')))
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		= execute_sql(" GRANT execute ON FUNCTION sp_subkontod_report(varchar, int4, date, date, int, varchar, int) TO GROUP dbkasutaja " )
		= execute_sql(" GRANT execute ON FUNCTION  sp_subkontod_report(varchar, int4, date, date, int, varchar, int) TO GROUP dbpeakasutaja " )		
		ENDIF
		
	Endif

	Wait Window 'Stored procedure: sp_calc_tasu' Nowait
	lcFile = 'tmp/fnc_sp_calc_tasu.sql'
	If File(lcFile)


		If !Empty(CHECK_obj_pg('PROC',UPPER('sp_calc_tasu')))
			lcString = " drop function sp_calc_tasu (int4, int4, date)"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		= execute_sql(" GRANT execute ON FUNCTION  sp_calc_tasu(int4, int4, date) TO GROUP dbkasutaja " )
		= execute_sql(" GRANT execute ON FUNCTION  sp_calc_tasu(int4, int4, date) TO GROUP dbpeakasutaja " )		
	Endif
