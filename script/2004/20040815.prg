...
* menu records
	...
	Select menupohi
	Locate For Alltrim(Upper(Pad)) = Alltrim(Upper('Operation')) And Alltrim(Bar) = '8'
	If !Found()
		lcOmandus = 'RUS CAPTION=Расчет пени'+Chr(13)+'EST CAPTION=Viivise arvestamine'
		lcproc = "=nObjekt('do form viivis ')"

		Insert Into menupohi (Pad, Bar, proc_, omandus, level_) Values ;
			('Operation','8',lcproc, lcOmandus, 1)
...
Function _alter_pg
	...
	If Empty(CHECK_obj_pg('TABLE','VANEMTASU1'))
&& создаем новуб таблицу
		lcString = " Create Table vanemtasu1 (id SERIAL NOT NULL PRIMARY KEY, isikkood VARCHAR(20) DEFAULT SPACE(1) , "+;
			" nimi VARCHAR(254) DEFAULT SPACE(1), vanemkood VARCHAR(20) DEFAULT SPACE(1), vanemnimi VARCHAR(254) DEFAULT SPACE(1),"+;
			" aadress TEXT DEFAULT SPACE(1), muud TEXT DEFAULT SPACE(1)   )"

		If execute_sql(lcString) < 0
			Return .F.
		Endif

		= execute_sql(" CREATE INDEX vanemtasu1_isikkood  ON public.vanemtasu1  USING btree  (isikkood)")

		= execute_sql("CREATE SEQUENCE public.vanemtasu1_id_seq  INCREMENT 1  MINVALUE 1  MAXVALUE 9223372036854775807  START 9585  CACHE 1")

		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu1 TO GROUP dbkasutaja " )
		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu1 TO GROUP dbpeakasutaja ")
		= execute_sql(" GRANT SELECT ON TABLE vanemtasu1 TO GROUP dbvaatleja ")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu1_id_seq TO GROUP dbkasutaja")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu1_id_seq TO GROUP dbpeakasutaja")

	Endif


	If Empty(CHECK_obj_pg('TABLE','VANEMTASU2'))
		lcString = " Create Table vanemtasu2 (id SERIAL NOT NULL PRIMARY KEY , isikId int NOT NULL, tunnus VARCHAR(20) DEFAULT SPACE(1),"+;
			" rekvid int NOT NULL, algkpv date NOT NULL DEFAULT DATE(), loppkpv date NOT NULL DEFAULT DATE(YEAR(DATE()),MONTH(DATE()), DAY(DATE())) ,"+;
			" jaak numeric (14,4) default 0, grupp varchar(40) default SPACE(1),  muud text default SPACE(1)) "

		If execute_sql(lcString) < 0
			Return .F.
		Endif
		= execute_sql(" CREATE INDEX vanemtasu2_isikId  ON public.vanemtasu2  USING btree  (isikid)")
		= execute_sql(" CREATE INDEX vanemtasu2_rekvid  ON public.vanemtasu2  USING btree  (rekvid)")
		= execute_sql(" CREATE INDEX vanemtasu2_kpv  ON public.vanemtasu2  USING btree  (algkpv, loppkpv)")

		= execute_sql("CREATE SEQUENCE public.vanemtasu2_id_seq  INCREMENT 1  MINVALUE 1  MAXVALUE 9223372036854775807  START 9585  CACHE 1")


		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu2 TO GROUP dbkasutaja " )
		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu2 TO GROUP dbpeakasutaja ")
		= execute_sql(" GRANT SELECT ON TABLE vanemtasu2 TO GROUP dbvaatleja ")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu2_id_seq TO GROUP dbkasutaja")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu2_id_seq TO GROUP dbpeakasutaja")

	Endif
	If Empty(CHECK_obj_pg('TABLE','VANEMTASU3'))

		lcString = " Create Table vanemtasu3 (id serial not null PRIMARY KEY , rekvid int not null , "+;
			" journalId int default 0 not null, dokpropId int default 0 not null, USERID INT not null default 0,"+;
			" opt int DEFAULT 1 not null , kpv date default DATE() not null, konto varchar(20) not null default SPACE(1),"+;
			" tp varchar(20) not null default '800699' , summa numeric (14,4) default 0 not null, "+;
			" tunnus varchar(20) default SPACE(1) not null , muud text default SPACE(1)) "

		If execute_sql(lcString) < 0
			Return .F.
		Endif

		= execute_sql(" CREATE INDEX vanemtasu3_isikId  ON public.vanemtasu3  USING btree  (rekvId,userid)")
		= execute_sql(" CREATE INDEX vanemtasu3_journalId  ON public.vanemtasu3  USING btree  (journalId, dokpropId)")
		= execute_sql(" CREATE INDEX vanemtasu3_kpv  ON public.vanemtasu3  USING btree  (kpv)")
		= execute_sql(" CREATE INDEX vanemtasu3_opt  ON public.vanemtasu3  USING btree  (opt, tunnus)")

		= execute_sql("CREATE SEQUENCE public.vanemtasu3_id_seq  INCREMENT 1  MINVALUE 1  MAXVALUE 9223372036854775807  START 9585  CACHE 1")


		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu3 TO GROUP dbkasutaja " )
		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu3 TO GROUP dbpeakasutaja ")
		= execute_sql(" GRANT SELECT ON TABLE vanemtasu3 TO GROUP dbvaatleja ")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu3_id_seq TO GROUP dbkasutaja")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu3_id_seq TO GROUP dbpeakasutaja")

	Endif
	If Empty(CHECK_obj_pg('TABLE','VANEMTASU4'))

		lcString = " Create Table vanemtasu4 (id serial PRIMARY KEY not null , parentId int not null , isikId int not null, "+;
			" maksjakood varchar(20) not null, maksjanimi varchar (254) not null, "+;
			" NomId int default 0 not null, kogus numeric (14,4) default 0 not null, hind numeric (14,4) default 0 not null,"+;
			" summa numeric (14,4) not null, konto varchar(20) default SPACE(1) not null, tp varchar (20) not null default '800699' ,"+;
			" kood1 varchar(20) not null default SPACE(1), kood2 varchar(20) not null default SPACE(1) ,"+;
			" kood3 varchar(20) not null default SPACE(1), kood4 varchar(20) not null default SPACE(1),"+;
			" kood5 varchar(20) not null default SPACE(1), number varchar(20) not null default SPACE(1),  muud text default SPACE(1)) "

		If execute_sql(lcString) < 0
			Return .F.
		Endif

		= execute_sql(" CREATE INDEX vanemtasu4_isikId  ON public.vanemtasu4  USING btree  (isikid, parentid)")
		= execute_sql(" CREATE INDEX vanemtasu4_nomId  ON public.vanemtasu4  USING btree  (nomId)")
		= execute_sql(" CREATE INDEX vanemtasu4_number  ON public.vanemtasu4  USING btree  (number)")

		= execute_sql("CREATE SEQUENCE public.vanemtasu4_id_seq  INCREMENT 1  MINVALUE 1  MAXVALUE 9223372036854775807  START 9585  CACHE 1")


		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu4 TO GROUP dbkasutaja " )
		= execute_sql(" GRANT INSERT,SELECT,UPDATE,DELETE ON TABLE vanemtasu4 TO GROUP dbpeakasutaja ")
		= execute_sql(" GRANT SELECT ON TABLE vanemtasu4 TO GROUP dbvaatleja ")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu4_id_seq TO GROUP dbkasutaja")
		= execute_sql("GRANT ALL ON TABLE public.vanemtasu4_id_seq TO GROUP dbpeakasutaja")


*!*			CREATE TRIGGER ON vanemtasu4   FOR INSERT  AS trigiu_vanemtasu4()
*!*			CREATE TRIGGER ON vanemtasu4   FOR UPDATE  AS trigiu_vanemtasu4()
*!*			CREATE TRIGGER ON vanemtasu4   FOR DELETE  AS trigd_vanemtasu4()
	...
	Wait Window 'Stored procedure: sp_calc_vanemtasu_jaak' Nowait


	lcFile = 'tmp/fnc_sp_calc_vanemtasu_jaak.sql'
	If File(lcFile)


		If !Empty(CHECK_obj_pg('PROC',UPPER('sp_calc_vanemtasu_jaak')))
			lcString = " drop function sp_calc_vanemtasu_jaak (int,int)"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		= execute_sql(" GRANT execute ON FUNCTION  sp_calc_vanemtasu_jaak(INT,INT) TO GROUP dbkasutaja " )
		= execute_sql(" GRANT execute ON FUNCTION  sp_calc_vanemtasu_jaak(INT,INT) TO GROUP dbpeakasutaja " )

	Endif

	Wait Window 'Stored procedure: sp_del_vanemtasu' Nowait


	lcFile = 'tmp/fnc_sp_del_vanemtasu.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('sp_del_vanemtasu')))
			lcString = " drop function sp_del_vanemtasu (int,int)"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		= execute_sql(" GRANT execute ON FUNCTION  sp_del_vanemtasu(INT,INT) TO GROUP dbkasutaja " )
		= execute_sql(" GRANT execute ON FUNCTION  sp_del_vanemtasu(INT,INT) TO GROUP dbpeakasutaja " )

	Endif


	lcFile = 'tmp/trigiu_vanemtasu1_after.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('trigiu_vanemtasu1_after')))
			lcString = " drop function trigiu_vanemtasu1_after() cascade"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
	...
	lcFile = 'tmp/trigd_vanemtasu1_before.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('trigd_vanemtasu1_after')))
			lcString = " drop function trigd_vanemtasu1_before() cascade"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
	...
	lcFile = 'tmp/trigd_vanemtasu4_after.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('trigd_vanemtasu4_after')))
			lcString = " drop function trigd_vanemtasu4_after() cascade"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
	...
	lcFile = 'tmp/trigi_vanemtasu4_after.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('trigi_vanemtasu4_after')))
			lcString = " drop function trigi_vanemtasu4_after() cascade"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
	...
	lcFile = 'tmp/trigu_vanemtasu4_after.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('trigu_vanemtasu4_after')))
			lcString = " drop function trigu_vanemtasu4_after() cascade"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
	...
	lcFile = 'tmp/trigiu_vanemtasu3_after.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('trigiu_vanemtasu3_after')))
			lcString = " drop function trigiu_vanemtasu3_after() cascade"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
	...
	lcFile = 'tmp/trigd_vanemtasu3_after.sql'
	If File(lcFile)

		If !Empty(CHECK_obj_pg('PROC',UPPER('trigd_vanemtasu3_after')))
			lcString = " drop function trigd_vanemtasu3_after() cascade"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
		endif
	...
	lcString = " select id from menupohi where UPPER(pad) = 'OPERATION' and bar = '8'"
	...
	lcString = " select id from menupohi where UPPER(pad) = UPPER('Asutused') and bar = '9'"
	lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
	...
	If Reccount('qrymenupohi') < 1
		...
		lcString = "Insert Into menupohi (Pad, Bar, proc_, omandus, level_) Values "+;
			"('Asutused','9','"+lcproc+"','"+lcOmandus+"', 2)"
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		lcString = " select id from menupohi where UPPER(pad) = 'ASUTUSED' and bar = '9'"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
	...
	lcString = " select id from menupohi where UPPER(pad) = UPPER('Lapsed') and EMPTY(bar)"
	lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
...
*		[=nObjekt("do form viivis ")]
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		lcString = " select id from menupohi where UPPER(pad) = 'LAPSED' and EMPTY(bar)"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		lcString = " select id from menupohi where UPPER(pad) = 'LAPSED' and bar = '1'"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		lcString = " select id from menupohi where UPPER(pad) = 'LAPSED' and bar = '2'"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		lcString = " select id from menupohi where UPPER(pad) = 'LAPSED' and bar = '3'"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		lcString = " select id from menupohi where UPPER(pad) = 'LAPSED' and bar = '4'"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
		...
		If execute_sql(lcString) < 0
			Return .F.
		Endif
		lcString = " select id from menupohi where UPPER(pad) = 'LAPSED' and bar = '5'"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
	...
	lcString = " select id from menupohi where UPPER(pad) = UPPER('Library') and bar = '41'"
	lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
...
*		[=nObjekt("do form viivis ")]
		...
		lcString = " select id from menupohi where UPPER(pad) = UPPER('VANEMTASU') and bar = '5'"
		lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')

		If Reccount('qrymenupohi') < 1
			lcOmandus = 'RUS CAPTION=Расчет'+Chr(13)+'EST CAPTION=Vanemtasu arvestamine'
			lcproc = "gcWindow.vanemtasuarv"
*		[=nObjekt("do form viivis ")]

			lcString = "Insert Into menupohi (Pad, bar,  proc_, omandus, level_) Values "+;
				"('Vanemtasu','5','"+lcproc+"','"+lcOmandus+"', 1)"
			If execute_sql(lcString) < 0
				Return .F.
			Endif
