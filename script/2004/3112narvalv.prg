...
*!*		Endif
*!*		If lError > 0
*!*			lError = volikogu_import()
*!*		Endif
*!*		If lError > 0
*!*			lError = rugodiv_import()
*!*		Endif
*!*		If lError > 0
*!*			lError = kassalib_import()
*!*		Endif
	Set Step On
	If lError > 0
		lError = korr_katselei_pv()
	Endif
*!*		If lError > 0
*!*			lError = pv_varaamet_import_1111()
*!*		Endif
*!*		If lError > 0
*!*			lError = pv_varaamet_import_1101()
*!*		Endif

*!*		If lError > 0
*!*			lError = pv_varaamet_import_1091()
*!*		Endif
*!*		If lError > 0
*!*			lError = pv_varaamet_import_1141()
*!*		Endif
	Return lError
Endfunc

Function korr_katselei_pv
	Local lError
	lError = 0
	lcData = 'C:\avpsoft\files\asset2\dbase\assetdata.dbc'
	If !File(lcData)
		Messagebox('DBC ei leidnud')
		Return .F.
	Endif
	...
	Select asset
	Scan
		Wait Window asset.Code +Str(Recno('asset'))+'-'+Str(Reccount('asset')) Nowait
		If !Used('cl')
			Use cl In 0
		Endif
		Select cl
		Locate For coode = asset.coode
		lcNimetus = Trim(cl.client)
		lcString = "select id from asutus where UPPER(nimetus) = UPPER( '"+lcNimetus+"')"
		lError = sqlexec(gnhandle, lcString,'qryVId')
		...
		If lError < 1
			...
			Exit
		Endif
		If Reccount('qryVId') < 1
* didn't found
			lcvastI = '1450'
		Else
			lcvastI = Str(qryVId.Id)
		Endif

		IF asset.code = '000078'
			SET STEP ON 
		endif
* 1. check for exist.
		lcString = "select * from curpohivara where rekvId = 3 and kood = '"+asset.Code+"'"
		...
		If Reccount('qryPv') < 1
			WAIT WINDOW asset.Code +'puudub' TIMEOUT 1
* pv puudub, on vaja lisada

*!*				lcString = " insert into library (rekvid,kood,nimetus,library) values ( 3,'"+;
*!*					Ltrim(Rtrim(asset.Code))+"','"+;
*!*					LTRIM(Rtrim(asset.Description))+"','POHIVARA')"
...
*!*				If !Used('qryPvGrupp')
*!*					lcString = "select * from library where rekvId = 3 and library = 'PVGRUPP'"
*!*					lError = sqlexec(gnhandle, lcString,'qryPvGrupp')
*!*					If lError < 0
*!*						Messagebox("Viga "+lcString,'Kontrol')
*!*						Set Step On
*!*						_Cliptext = lcString
...
*!*				Endif
*!*				Select qryPvGrupp
*!*				Locate For Alltrim(Upper(kood)) = Alltrim(Upper(asset.Type))
*!*				If !Found()
*!*					Messagebox("Viga, grupp ei leidn. "+asset.Type,'Kontrol')
*!*					lError = -1
*!*					Exit
*!*				Endif

*!*				lcSoetKpv = "date ("+Str(Year(asset.datexpl),4)+","+Str(Month(asset.datexpl),2)+","+STR(Day(asset.datexpl),2)+")"
*!*				lcString = " insert into pv_kaart (parentid,  vastisikid,  soetmaks,soetkpv,  kulum  ,"+;
*!*					" gruppid,  tunnus, MUUD ) values ("+;
*!*					Str( qryLastid.Id)+",12,"+lcvastI+","+lcSoetKpv+","+Str(asset.amort)+","+;
*!*					STR(qryPvGrupp.Id)+",1,'"+Trim(asset.location)+"')"
...
*!*				lcString = 'insert into pv_oper (parentId, nomId, liik, kpv, summa ) values ('+;
*!*					Str(qryLastid.Id)+',450,1,'+lcSoetKpv+','+Str(asset.algHIND,12,2)+')'
*!*				lError = sqlexec(gnhandle, lcString)
*!*				If lError < 1
*!*					= Aerror(err)
*!*					lcError = err(1,3)
...
*!*				Select * From qryLastid Into Cursor qryPv
*!*			Else
*!*				If !Empty(asset.location) AND asset.num <> 795
*!*	* rentnik
*!*	*				" algkulum = "+STR(asset.alghind - asset.osthind,12,2)+","+;


*!*					lcString = " UPDATE pv_kaart set muud = '"+Trim(asset.location)+"',"+;
*!*					" soetmaks = "+STR(asset.alghind,12,2)+","+;
*!*					" vastisikid = "+lcvastI+;
*!*					" where parentid = "+Str(qryPv.Id)
*!*					lError = sqlexec(gnhandle, lcString)
*!*					If lError < 1
*!*						Set Step On
*!*						_Cliptext = lcString
...
*!*				Endif
		Endif
		If asset.tunnus > 0
* mahakantud, uus operatsioon
			If  asset.num = 795
				lcSumma = '0'
			Else
				lcSumma = Str(asset.alghind,12,2)
			Endif
			lcString = 'insert into pv_oper (parentId, nomId, liik, kpv, summa ) values ('+;
				Str(qryPv.Id)+',301,4,DATE(2003,12,31),'+lcSumma+')'
			lError = sqlexec(gnhandle, lcString)
			If lError < 1
				Set Step On
				_Cliptext = lcString
...
Function pv_varaamet_import_kantselei
*	lcFile = 'c:\temp\vara\PVVara1111.dbf'
	...
	Import From c:\avpsoft\Files\PVKANTSELEI.Xls Type Xls

	Go Top
	If PVKANTSELEI.a = 'num'
		Delete Next 1
	Endif

	Select Distinct m As grupp From PVKANTSELEI Where Alltrim(a) <> 'num' Into Cursor qryGrupp

	Create Cursor mssqlgrupplib(Id Int,rekvId Int ,kood c(20),nimetus c(254),Library c(20) Default 'PVGRUPP',muud m,tun1 Int,;
		tun2 Int, tun3 Int,  tun4 Int,  tun5 Int)

	Insert Into mssqlgrupplib (rekvId, kood, nimetus );
		SELECT lnrekvid As rekvId, qryGrupp.grupp, qryGrupp.grupp From qryGrupp
...
*	Use (lcFile) In 0 Alias v_pvvara
	...
	Select PVKANTSELEI
	Scan For a <> 'num'
		Wait Window PVKANTSELEI.r Nowait
		Select mssqlgrupplib
		Locate For Alltrim(kood) = Alltrim(PVKANTSELEI.m)
* kontrolime inv.number
		lcString = "select id from curPohivara where kood = '"+Trim(PVKANTSELEI.r)+"' and rekvid = "+;
			STR(lnrekvid)
...
*  uus pv
			lcString = " insert into library (rekvid,kood,nimetus,library) values ("+;
				Str(lnrekvid)+",'"+Ltrim(Rtrim(PVKANTSELEI.r))+"','"+;
				LTRIM(Rtrim(PVKANTSELEI.b))+"','POHIVARA')"
			...
			lcString = " insert into pv_kaart (parentid,  vastisikid,  soetmaks,soetkpv,  kulum ,  algkulum ,"+;
				" gruppid,  tunnus, MUUD ) values ("+;
				Str( qryLastid.Id)+",12,"+;
				PVKANTSELEI.H+",DATE(2004,01,01),"+;
				PVKANTSELEI.G+" ,"+  Str(Val(Alltrim(PVKANTSELEI.H)) - Val(Alltrim(PVKANTSELEI.L)),14,2)+" ,"+;
				STR(mssqlgrupplib.Id)+",1,'"+PVKANTSELEI.N+"')"
...
* parandamine
			...
			lcString = " update pv_kaart set  soetmaks = "+PVKANTSELEI.H+","+;
				" algkulum = "+Str(Val(Alltrim(PVKANTSELEI.H)) - Val(Alltrim(PVKANTSELEI.L)),14,2)+","+;
				" MUUD = '"+PVKANTSELEI.N+"'"+;
				" where parentid = "+Str(qryInvKontrol.Id)
