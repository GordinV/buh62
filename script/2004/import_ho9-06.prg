...
lcFile1 = 'c:\avpsoft\haridus\lapsed\lapsed08_k.dbf'
...
* arv
		If lnresult > 0
			If !Empty(VAL(transdigit(ALLTRIM(qryHO.n48)))) And lnresult > 0
				lcDate = "date(2004,08,"+alltrim(qryHo.n16)+")"
				lcString = "insert into vanemtasu3 (rekvid, userid ,  opt ,  kpv ,  tunnus ,  muud ) values ("+;
					" 31, 222, 1,"+lcdate +", '"+Ltrim(Rtrim(qryHO.n1))+"','imp. fail DDU09_vlad.xls ')"
				lnresult =SQLEXEC(gnHandle,lcString)
				If lnresult > 0
...
*!*				Endif
		...
		If lnresult < 1
			...
			Exit
		Endif
		lnSumma = lnSumma + VAL(transdigit(ALLTRIM(qryHO.n48)))
		lnrec = lnrec + 1
	Endscan
