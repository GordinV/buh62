...
Insert Into v_ametid (Osakond, kood, nimetus);
	SELECT Distinct Left(n1,7) As Osakond, Iif(Empty(n69),LEFT('R-MUUD',20),'R-'+Left(n69,20)) As kood, ;
...
* otsime amet, ja kui puudub insert
	...
	lcString = "SELECT library.id from library inner join palk_asutus on library.id = palk_asutus.ametid "+;
		" where library = 'AMET' and library.rekvid = 31 and kood = '"+Alltrim(v_ametid.kood)+"'" +;
		" and palk_asutus.osakondId = "+Str(v_ametid.OsakondId)
	lnresult = SQLEXEC(gnHandle,lcString, 'qryId')
	If lnresult < 1
		Set Step On
		Exit
	...
	If  Reccount('qryid') < 1
		...
		If lnresult < 1
			...
			Exit
		Else
			lcString = "SELECT id from library where library = 'AMET' and library.rekvid = 31 and kood = '"+Alltrim(v_ametid.kood)+"'" +;
	...
	Select n3 As nimetus, n73 As pank, n74 As aa, n72 As isikukood, n75 As algkpv, n71 As aadress, n5 As palk From qryHO ;
		...
		AND Alltrim(Iif(Empty(n69),LEFT('R-MUUD',20),'R-'+Left(n69,20))) = Alltrim(v_ametid.kood);
		AND QRYhO.N4 = 1;
