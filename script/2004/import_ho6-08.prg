...
lcFile1 = 'c:\avpsoft\haridus\fakt\fakt9.dbf'
...
*!*		If Reccount('qryid') <  1 AND lcregkood <> '#N/A   '
...
Select qryHO
SCAN 
	*YEAR(qryHO.data) = 2004 AND !EMPTY(qryHo.kor_Konto) AND qryHo.kood_in_go <>'#N/A   '
	Wait Window Str(Recno('qryHo'))+'-'+Str(Reccount('qryHo')) Nowait
	lcErr = ''
...
*!*		Endif
	...
	lcAasta = STR(year(qryHo.kuup_ev),4)
	lcKuu = STR(month(qryHo.kuup_ev),2)
	lcPaev = str(day(qryHo.kuup_ev),2)
	lcdate = 'date('+lcAasta+','+lcKuu+','+lcPaev+')'
	
	
	lcuserId = '199'

	lcSelg = Ltrim(Rtrim(qryHO.selgitus))
	lcDok = ''
	lcMuud = ''
	lcUksus = ''

	lcDeebet = Left(Ltrim(RTRIM((qryHO.d_t_konto))),6)
	lcKreedit = Left(Ltrim(Rtrim(str(qryHO.k_t,9))),6)

	lcDeebet = Iif(lcDeebet = '501460','551460',lcDeebet)
	lcDeebet = Iif(lcDeebet = '551190','551109',lcDeebet)
	...
	lcKreedit = Iif(lcKreedit = '551190','551109',lcKreedit)
...
*	lcKreedit = Iif(lcKreedit = '382080','382090',lcKreedit)
	...
	lcSumma = str(qryHO.summa,14,2)
*	transdigit(Ltrim(Rtrim(qryHO.n9)))
	lcKood1 = Ltrim(Rtrim(qryHO.tt))
	lcKood2 = Ltrim(Rtrim(STR(qryHO.a,8)))
	lcKood3 = Ltrim(Rtrim(STR(qryHO.raha__voo,8)))
	lcKood4 = ''
	lcKood5 = Ltrim(Rtrim((qryHO.eelarve_os)))
	...
	lcTunnus = Alltrim(str(qryHO.asutuse_ko,9))
