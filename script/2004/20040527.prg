...
&& обновляем параметры отчетов
	...
	SELECT curPrinter
	LOCATE FOR LOWER(objekt) = 'eelarvearuanne' AND id = 382
	IF !FOUND()
		* lisamine
		APPEND BLANK
		replace id WITH 382,;
			objekt WITH 'EelarveAruanne',;
			nimetus1 WITH 'Бюджет (кассовые и факт. расходы)',;
			nimetus2 WITH 'Eelarve (kassa ja tegelik kulud)',;
			procfail WITH 'eelarve\eelarve_report8',;
			reportfail WITH 'eelarve\eelarve_report8',;
			reportvene WITH 'eelarve\eelarve_report8' IN curprinter
	ENDIF
...
* menu records
	...
	Select menupohi
	Locate For ALLTRIM(UPPER(Pad)) = ALLTRIM(UPPER('Aruanne')) And ALLTRIM(Bar) = '382'
	If !Found()
		lcOmandus = 'RUS CAPTION=Бюджет расходов(касса и факт)'+Chr(13)+'EST CAPTION=Eelarve kulud(kassa ja tegelik)'
		lcproc = "=nObjekt('do form eelarvearuanne with 382')"

		Insert Into menupohi (Pad, Bar, proc_, omandus, level_) Values ;
			('Aruanne','382',lcproc, lcOmandus, 1)
