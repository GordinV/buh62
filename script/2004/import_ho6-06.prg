...
lcFile1 = 'c:\avpsoft\haridus\fakt\fakt6.dbf'
...
Select qryHO
SCAN FOR YEAR(qryHO.data) = 2004 AND !EMPTY(qryHo.kor_Konto) AND qryHo.kood_in_go <>'#N/A   '
	Wait Window Str(Recno('qryHo'))+'-'+Str(Reccount('qryHo')) Nowait
	lcErr = ''
...
*	lcKreedit = Iif(lcKreedit = '382080','382090',lcKreedit)
	...
	lcTpd = Ltrim(Rtrim((qryHO.tp)))
	lcTpk = Ltrim(Rtrim((qryHO.tp)))
