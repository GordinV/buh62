...
lcFile1 = 'c:\avpsoft\haridus\fakt\fakt10.dbf'
...
Select qryHO
SCAN FOR !EMPTY(qryHo.kor_konto)
...
*!*		Endif
	...
	IF RECCOUNT('qryId') > 0
		lcAsutusId = STR(qryId.ID,11)
	ENDIF
	
	lcAasta = LEFT(STR(qryHo.period,6),4)
	lcKuu = RIGHT(STR(qryHo.period,6),2)
	lcPaev = '01'
	...
	lcDeebet = Left(Ltrim(RTRIM(str(qryHO.konto,9))),6)
	lcKreedit = Left(Ltrim(Rtrim((qryHO.kor_konto))),6)
