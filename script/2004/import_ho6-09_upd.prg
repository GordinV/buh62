...
*	lcregkood = Ltrim(Rtrim(qryHO.kood_in_go))
	...
	If Upper(lcAsutus) = Upper('Aripaeva Kirjastuse AS')
		lcAsutus = 'ÄRIPÄEVA KIRJASTUSE'
	ENDIF
	If Upper(lcAsutus) = Upper('Travibest Reisiburoo') OR Upper(lcAsutus) = Upper('Travibest Reisiburoo OU') OR Upper(lcAsutus) = Upper('Reisibüroo Travibest')
		lcAsutus = 'Travibest'
	ENDIF
