...
* arv
		If lnresult > 0
* alg.jaak
*!*				If !Empty(qryHO.algs_do__1)
*!*					lcString = "insert into vanemtasu3 (rekvid, userid ,  opt ,  kpv ,  tunnus ,  muud ) values ("+;
*!*						" 31, 222, 2, DATE(2004,07,01), '"+Ltrim(Rtrim(qryHO.kood))+"','alg.jääk')"
*!*					lnresult =SQLEXEC(gnHandle,lcString)
...
*!*							LTRIM(Rtrim(qryIsikId.vanemnimi))+"',855,1,"+Str(qryHO.algs_do__1,12,2)+","+Str(qryHO.algs_do__1,12,2)+")"
*!*						lnresult =SQLEXEC(gnHandle,lcString)
*!*					Endif
*!*				Endif
* arv 07 kuu eest

			If lnresult > 0 AND (qryHO.s1a + qryHO.s1 + qryHO.s1u ) <> 0

				lcString = "insert into vanemtasu3 (rekvid, userid ,  opt ,  kpv ,  tunnus ,  muud ) values ("+;
					" 31, 222, 2, DATE(2004,07,31), '"+Ltrim(Rtrim(qryHO.kood))+"','arv. 07/2004 kuu eest')"
				lnresult =SQLEXEC(gnHandle,lcString)
				If lnresult > 0
					lcString = " SELECT id FROM vanemtasu3 ORDER BY id desc limit 1"
					lnresult =SQLEXEC(gnHandle,lcString,'qryDokId')

					If !Empty(qryHO.s1) And lnresult > 0
						lcString = " insert into vanemtasu4 (parentid,isikid , maksjakood , maksjanimi ,  nomid ,  kogus ,  hind ,  summa ) values ("+;
							STR(qryDokId.Id,9)+","+Str(qryIsikId.Id,9)+",'"+Ltrim(Rtrim(qryIsikId.vanemkood))+"','"+;
							LTRIM(Rtrim(qryIsikId.vanemnimi))+"',852,1,"+Str(qryHO.s1,12,2)+","+Str(qryHO.s1,12,2)+")"
						lnresult =SQLEXEC(gnHandle,lcString)
					Endif
				Endif
				...
				If !Empty(qryHO.s1a ) And lnresult > 0
					lcString = " insert into vanemtasu4 (parentid,isikid , maksjakood , maksjanimi ,  nomid ,  kogus ,  hind ,  summa ) values ("+;
						STR(qryDokId.Id,9)+","+Str(qryIsikId.Id,9)+",'"+Ltrim(Rtrim(qryIsikId.vanemkood))+"','"+;
						LTRIM(Rtrim(qryIsikId.vanemnimi))+"',853,1,"+Str(qryHO.s1a,12,2)+","+Str(qryHO.s1a,12,2)+")"
					lnresult =SQLEXEC(gnHandle,lcString)

				Endif
				If !Empty(qryHO.s1u ) And lnresult > 0
					lcString = " insert into vanemtasu4 (parentid,isikid , maksjakood , maksjanimi ,  nomid ,  kogus ,  hind ,  summa ) values ("+;
						STR(qryDokId.Id,9)+","+Str(qryIsikId.Id,9)+",'"+Ltrim(Rtrim(qryIsikId.vanemkood))+"','"+;
						LTRIM(Rtrim(qryIsikId.vanemnimi))+"',855,1,"+Str(qryHO.s1u,12,2)+","+Str(qryHO.s1u,12,2)+")"
					lnresult =SQLEXEC(gnHandle,lcString)
		...
		If lnresult < 1
			...
			Exit
		Endif
		lnSumma = lnSumma + qryHO.s1+qryHO.isa+qryHO.isu
		lnrec = lnrec + 1
	Endscan
