...
lcFile1 = 'c:\avpsoft\haridus\fakt\fakt5.dbf'
...
*!*	* otsime asutusId
*!*		lcregkood = Ltrim(Rtrim(STR(qryHO.n6,11)))
...
Select qryHO
SCAN FOR YEAR(qryHO.n16) = 2004 AND  !EMPTY(qryHo.n1)
	Wait Window Str(Recno('qryHo'))+'-'+Str(Reccount('qryHo')) Nowait
	lcErr = ''
...
*!*			Endif
	...
	lcdate = 'date('+Str(Year(qryHO.n16),4)+','+Str(Month(qryHO.n16),2)+','+Str(Day(qryHO.n16),2)+')'

	lcuserId = '199'

	lcSelg = Ltrim(Rtrim(qryHO.n7))+Space(1)+Ltrim(Rtrim(STR(qryHO.n13)))
	lcDok = ''
	lcMuud = ''
	lcUksus = ''

	lcDeebet = Left(Ltrim(Rtrim(Str(qryHO.n2))),6)
	lcKreedit = Left(Ltrim(Rtrim((qryHO.n1))),6)
...
*	lcKreedit = Iif(lcKreedit = '382080','382090',lcKreedit)
	...
	lcTpd = Ltrim(Rtrim(Str(qryHO.n9)))
	lcTpk = Ltrim(Rtrim(Str(qryHO.n9)))
	lcSumma = Alltrim(Str(qryHO.n3,14,2))
*	transdigit(Ltrim(Rtrim(qryHO.n9)))
	lcKood1 = Ltrim(Rtrim(qryHO.n11))
	lcKood2 = ''
	lcKood3 = ''
	lcKood4 = ''
	lcKood5 = Ltrim(Rtrim(STR(qryHO.n12)))
