...
lError = 1
if !USED('majandus2008')
	USE DOK/majandus2008 alias nlvko2008 IN 0 
ENDIF
Set Step On
...
*lError = fnc_pv()
	...
	lError = fnc_eelarve(6, '','')
*!*	lError = fnc_yksused()
*!*		lError = 1
*!*	*	lcKood = UPPER(LEFT(LTRIM(sqlrekv.nimetus),7))
*!*		lcKood = '0922075'
*!*		lnOsakondId = fnc_osakonna_struktuur(119,lcKood)
*!*		IF !EMPTY(lError) AND lnOsakondId > 0
*!*			lError = fnc_ameti_struktuur(119,lnOsakondId, lcKood)
*!*	*!*		ELSE
*!*	*!*			SET STEP ON 
*!*		ENDIF
*!*		IF !EMPTY(lError) AND lnOsakondId > 0
*!*			lError = fnc_tooleping(119, lnOsakondId, lcKood)
*!*		ENDIF
*!*		IF !EMPTY(lError) AND lnOsakondId > 0
*!*			lError = fnc_palgaStruktuur(119,lcKood)
*!*		ENDIF
*!*		IF !EMPTY(lError) AND lnOsakondId > 0
*!*			lError = fnc_palgaKaart(119, lcKood)
*!*		ENDIF
*!*			lerror = fnc_analuus(119)
*!*	SELECT cur_analuus
*!*	brow
...
*	If !Used('sqlHKSPalgaKaart')
		lcString = "SELECT Palk_kaart.id, Palk_kaart.parentid, Palk_kaart.lepingid, Palk_kaart.libid, palk.kood as palk, "+;
			...
			" where tooleping.rekvid = 62 and osakond.kood = '"+tcOsakond+"'"
		...
		Endif
*	ENDIF
	lcString = "select * from library where rekvid = 119 and library = 'PALK'"
		...
		If lnError < 0
			...
			*set step on
			Return lnError
		Endif
	lcString = "select * from library where rekvid = 119 and library = 'OSAKOND' and kood = '"+tcOsakond+"'"
		...
		If lnError < 0
			...
			*set step on
			Return lnError
		Endif
	lcString = "select * from library where rekvid = 119 and library = 'AMET'"
	...
	lcString = "select * from tooleping where rekvid = 119 and osakondId = "+STR(sqlUusOsakond.id,9)
	...
	Select Distinct  Id, parentid, lepingid, libid From sqlHKSPalgaKaart Where osakond = tcOsakond Into Cursor tmpKaardid
	Scan
...
*!*	ENDIF
	...
	If !Used('sqlHKSPalgaKaart')
		lcString = "SELECT Palk_kaart.id, Palk_kaart.parentid, Palk_kaart.lepingid, Palk_kaart.libid, Tooleping.osakondid, Tooleping.ametid "+;
			" FROM palk_kaart INNER JOIN tooleping  ON  Palk_kaart.lepingid = Tooleping.id  "+;
			" inner join library osakond on osakond.id = tooleping.osakondId where tooleping.rekvid = 62 and osakond.kood = '"+tcKood +"'";
	...
	Select Distinct libid From sqlHKSPalgaKaart Into Cursor tmpPalkLib
...
*	If !Used('sqlHKSToolepingud')
		...
		lcString = "select tooleping.*, osakond.kood as osakond, amet.kood as amet "+;
			...
			" where tooleping.rekvId = 62 and osakondid in (select id from library where rekvid = 62 and library = 'OSAKOND' "+;
