	...
	* library palk
	...
	SCAN
		...
		* kontrollime kas kood juba salvestatud
		lcString = "select id from library where LTRIM(RTRIM(kood)) = '"+alLTRIM(tmpLib.kood)+"' and rekvid = 67 and library = 'PALK'"
		...
		IF RECCOUNT('tmpId') = 0 
			* puudub, lisame
			...
			lcString = "insert into library (rekvid, kood, nimetus, library, tun1, tun2, tun3, tun4) values (67,'"+;
