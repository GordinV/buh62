...
*SET STEP ON 
gnHandle2009 = SQLConnect('meke')
gnHandle2008 = SQLConnect('meke2010')
If gnHandle2009 < 1
	...
	* Tellime saldoandmik
	...
	If Used('qrySA')
		...
		lcString = "select konto, tp, tegev, allikas, rahavoo, sum(db) as db, sum(kr) as kr, nimetus from tmp_saldoandmik where rekvid = "+str(qryRekv.id)+;
			...
			" left(konto,6) not in ('103000','201000','201010') "+;
...
*!*		endif
	...
	lcString = "select sp_subkontod_report('103000%',"+ Str(qryRekv.id)+","+lcKpv2+","+lcKpv2+",0,'%',3,0)"
	lError = sqlexec(gnhandle2008,lcString,'qrySA')
	tcTimestamp = Alltrim(qrySA.sp_subkontod_report)

	wait window lcAsutuseNimi+' subkontod 10...' nowait 
	
	lcString = "select algjaak, konto, asutusId from tmp_subkontod_report where left(konto,6) in('103000') and timestamp = '"+tcTimestamp+"' order by konto, asutusid" 
...
*	brow
	...
	lcString = "select sp_subkontod_report('201000%',"+ Str(qryRekv.id)+","+lcKpv2+","+lcKpv2+",0,'%',3,0)"
	lError = sqlexec(gnhandle2008,lcString,'qrySA')
	tcTimestamp = Alltrim(qrySA.sp_subkontod_report)

	
	lcString = "select algjaak, konto, asutusId from tmp_subkontod_report where left(konto,6) in('201000') and timestamp = '"+tcTimestamp+"' order by konto, asutusid" 
	lError = sqlexec(gnhandle2008,lcString,'qrySaldoKreedit1')

	lcString = "select sp_subkontod_report('201010%',"+ Str(qryRekv.id)+","+lcKpv2+","+lcKpv2+",0,'%',3,0)"
	lError = sqlexec(gnhandle2008,lcString,'qrySA')
	tcTimestamp = Alltrim(qrySA.sp_subkontod_report)

	lcString = "select algjaak, konto, asutusId from tmp_subkontod_report where left(konto,6) in('201010') and timestamp = '"+tcTimestamp+"' order by konto, asutusid" 
...
*!*		
*!*		lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2010,12,31) and selg like 'Alg.saldo%'"
...
*		if left(qrySaldoAruanne.KONTO,3) <> '102' and left(qrySaldoAruanne.konto,3) <> '103'
			lcString = "insert into journal1 (parentid, summa,kood1,kood2,kood3,deebet,lisa_d,kreedit,lisa_k) values ("	+;
				str(qryId.id,9)+","+str(ROUND(qrySaldoAruanne.db/15.6466,2),14,2)+",'"+qrySaldoAruanne.tegev+"','"+qrySaldoAruanne.allikas+"','"+;
...
*!*			lcString = "insert into journal1 (parentid, summa,kood1,kood2,kood3,deebet,lisa_d,kreedit,lisa_k) values ("	+;
*!*				str(qryId.id,9)+","+str(ROUND(qrySaldoDeebet.algjaak/15.6466,2),14,2)+",'"+lcTegev+"','"+lcAllikas+"','"+;
...
*!*					str(qryId.id,9)+","+str(ROUND(qrySaldoAruanne.kr/15.6466,2),14,2)+",'"+qrySaldoAruanne.tegev+"','"+qrySaldoAruanne.allikas+"','"+;
*!*					qrySaldoAruanne.rahavoo+"','"+qrySaldoAruanne.konto+"','"+qrySaldoAruanne.tp+"','999999','"+qrySaldoAruanne.tp+"')"
...
*!*	*!*			endif
*!*			lcString = "insert into journal1 (parentid, summa,kood1,kood2,kood3,kreedit,lisa_k,deebet,lisa_d) values ("	+;
*!*				str(qryId.id,9)+","+ str(-1 * ROUND(qrySaldoKreedit.algjaak/15.6466,2),14,2)+",'"+lcTegev+"','"+lcAllikas+"','"+;
