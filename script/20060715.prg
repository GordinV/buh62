...
IF lnTbl > 0 AND ASCAN(aTbl,UPPER('ARV3')) = 0
	lcPath = Sys(5)+Sys(2003)
	IF UPPER(JUSTPATH(DBC())) <> UPPER(lcPath)
		Set Default To Justpath (DBC())
	ENDIF
	
	CREATE table arv3 (ID i PRIMARY KEY AUTOINC, arvid i, arv1id i, autoid i, isikid i, lastupd d DEFAULT DATE())
	INDEX ON arvid TAG arvid
	INDEX ON arv1id TAG arv1id
	INDEX ON autoid TAG autoid
...
Function _alter_pg
	...
	If v_account.admin > 0 Or v_account.peakasutaja > 0
		...
		lcFile = 'tmp/20060715.sql'

		IF File(lcFile) AND NOT CHECK_obj_pg('TABLE', UPPER('arv3'))
