...
Function import_tootajad
	...
	lcFile = 'c:\temp\paju.xls'
		lTulemus = .t.
	Import From (lcFile) Type Xls

	Select * From paju Into Cursor curTt

	Select curTt
	SCAN FOR !EMPTY(curTt.e)
* check for name in asutus
		lcIsikuKood = Alltrim(STR(curTt.c,12,0))
		lcNimi = Trim(curTt.b)
		lcAadress = Trim(curTt.d)
		lcAmet = Alltrim(curTt.f)
		lcOsakond = Alltrim(STR(curTt.j))
		lcAlgKpv = ''
		lcAa = Alltrim((curTt.e))
		lcpalk = LEFT(ALLTRIM(curTt.h),4)
		lcPohikoht = Alltrim(STR(curTt.j,1))
		lcpank = Iif(Left(lcAa,2) = '33','722',Iif(Left(lcAa,4) = '1056','401','767'))
		lcKoormus = Alltrim(curTt.i)
		...
		If get_tooleping(lnisikid, lnOsakond,lnAmetid) = 0
			=uusTooleping(lnisikid, lnOsakond,lnAmetid)
*!*					If !err()
*!*						Exit
*!*					Endif
*!*				Endif
