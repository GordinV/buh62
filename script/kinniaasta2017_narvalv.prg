...
gnHandle2008 = SQLConnect('narvalv2016','vlad','Vlad490710')
If gnHandle2009 < 1
...
* asutuse nimekirje
lcString = "select id, nimetus from rekv where parentid < 999 and id not in (select id from rekv where id in (63, 119) or parentid = 119)" 
*!*	and "+;
...
lcKpv = 'date(2017,01,01)'
lcKpv2 = 'date(2016,12,31)'
select qryRekv
scan
...
*		select * from qrySaldoKreedit1 into cursor qrySaldoKreedit
	...
	* kustutame vana lausend
	...
	lcString = "update aasta set kinni = 0 where rekvid = " + str(qryrekv.id,9)+ " and kuu = 12 and aasta = 2016"
	...
	lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2016,12,31) and selg ilike 'Alg.saldo%'"
	lError = sqlexec(gnhandle2009,lcString)
	if lError < 0
		messagebox('Viga.'+lcString)
		set step on
