...
select * from aasta where aasta = 2011 and kuu = 9

update aasta set kinni = 0 where aasta = 2011 and kuu = 9
select * from curPohivara where kood = '003554'

select count(id) from curpohivara where rekvid = 6 and tunnus = 1 and kulum = 0
select sp_pvkaardid_kopeerimine(id) from rekv where parentid < 200
select count(id) from curjournal where kpv = date(2011,09,30) 
*/
...
if ifnull(lnNomMahaId,0) = 0 then
	...
	lnNomMahaId = sp_salvesta_nomenklatuur(0, tnRekvId, 0, 'MAHAKANDMINE','PVMAHA', 'PV kaardid mahakandmine', '', 0, 'Rahandusameti korraldus',0, 0, '', 'EUR', 15.6466);
end if;
...
if ifnull(lnNomPaigId,0) = 0 then
	-- operatsioon puudub, lisame
	lnNomPaigId = sp_salvesta_nomenklatuur(0, tnRekvId, 0, 'PAIGUTUS','PVPAIG', 'PV kaardid paigaldamine', '', 0, 'Rahandusameti korraldus',0, 0, '', 'EUR', 15.6466);
end if;
...
for lrPvKaardid  in
	select * from curPohivara where rekvid = tnRekvId and tunnus = 1 and kulum = 0
loop
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION sp_pvkaardid_kopeerimine(integer) OWNER TO vlad;
