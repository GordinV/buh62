...
SCAN FOR id = 120
	...
	lnOsakondId = fnc_osakonna_struktuur(sqlrekv.id,lcKood)
	IF !EMPTY(lError) AND lnOsakondId > 0
		lError = fnc_ameti_struktuur(sqlrekv.id,lnOsakondId, lcKood)
*!*		ELSE
*!*			SET STEP ON 
	ENDIF
	IF !EMPTY(lError) AND lnOsakondId > 0
		lError = fnc_tooleping(sqlrekv.id, lnOsakondId)
	ENDIF
	IF !EMPTY(lError) AND lnOsakondId > 0
		lError = fnc_palgaStruktuur(sqlrekv.id)
	ENDIF
	IF !EMPTY(lError) AND lnOsakondId > 0
		lError = fnc_palgaKaart(sqlrekv.id)
	ENDIF
		lerror = fnc_analuus(sqlrekv.id)
...
Function fnc_palgaKaart
	...
	If !Used('sqlHKSPalgaKaart')
		Messagebox('Puudub sqlHKSPalgaKaart')
		Return 0
	Endif
	Select Distinct  Id, lepingid, libid From sqlHKSPalgaKaart Where lepingid In (Select vanaid From cur_tooleping) Into Cursor tmpKaardid
	Scan
		...
		If !Eof('cur_tooleping') And !Eof('cur_palklib')
			lcString = "select sp_kopeeri_palgakaart("+Str(tmpKaardid.Id,9)+","+Str(cur_tooleping.uutid,9)+","+Str(cur_palklib.uutid,9)+")"
			lnError = SQLEXEC(gnHandle,lcString,'tmpId')
			If lnError < 0
				Messagebox('Viga',0,'fnc_palgaKaart',10)
				*set step on
...
*!*	ENDIF
	...
	If !Used('sqlHKSPalgaKaart')
		lcString = "SELECT Palk_kaart.id, Palk_kaart.parentid, Palk_kaart.lepingid, Palk_kaart.libid, Tooleping.osakondid, Tooleping.ametid "+;
			" FROM palk_kaart INNER JOIN tooleping  ON  Palk_kaart.lepingid = Tooleping.id  where tooleping.rekvid = 62 "
...
Function fnc_tooleping
	Lparameters tnRekvId, tnOsakondId
...
* kustutame vana toolepingud
	...
	* otsime vanaosakondId
	lcOsakonnaKood = LEFT(LTRIM(sqlRekv.nimetus),7)

*	If !Used('sqlHKSToolepingud')
		Wait Window 'sqlHKSToolepingud .. ' Nowait
		lcString = "select * from tooleping where rekvId = 62 and osakondid in (select id from library where rekvid = 62 and library = 'OSAKOND' and kood = '"+lcOsakonnaKood+"')"
...
*	Endif
	...
	lnCount = 0
	Select cur_amet
	Scan For rekvid = tnRekvId

		Select sqlHKSToolepingud
		Scan For ametid = cur_amet.vanaAmetId
			lnCount = lnCount +1
			...
			lcString = "insert into Tooleping (parentid, osakondid, ametid, algab, lopp, toopaev, palgamaar, palk, pohikoht,koormus, ametnik,"+;
				...
				STR(sqlHKSToolepingud.parentId,9)+","+Str(tnOsakondId,9)+","+Str(cur_amet.uutAmetid,9)+","+;
