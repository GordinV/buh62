...
SET STEP ON 
gnHandle2009 = SQLConnect('njlv2010','vlad','Vlad490710')
gnHandle2008 = SQLConnect('njlv2009','vlad','Vlad490710')
If gnHandle2009 < 1
...
*and parentid <> 9999 and id <> 119 and parentid <> 119 " 
*!*	and "+;
	...
	* Tellime saldoandmik
	...
	If Used('qrySA')
		...
		lcString = "select konto, tp, tegev, allikas, rahavoo, sum(db) as db, sum(kr) as kr, nimetus from tmp_saldoandmik where rekvid = "+str(qryRekv.id)+;
			...
			" left(konto,3) not in ('102','103','109','108','201','203','206','208','258') "+;
			" and LEFT(konto,6) not in ('202000','202090') "+;
