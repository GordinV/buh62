	...
	Case gversia = 'VFP'
		...
		Scan For Mline(qryKey.Connect,1) = 'FOX'
			...
			If File (lcdata)
				...
				lcdataproc = lcdefault+'\tmp\buhdata5_20050202.sql'
				If File (lcdataproc)
...
CREATE SQL VIEW curPuudumine_ as;
SELECT puudumine.id, puudumine.lepingid, puudumine.kpv1, puudumine.kpv2, puudumine.paevad, puudumine.summa,  ;
puudumine.tunnus, puudumine.tyyp, amet.nimetus AS amet, amet.rekvid, asutus.regkood AS isikukood, ;
asutus.nimetus AS isik, IIF(puudumine.tunnus = 1, 'PUHKUS','HAIGUS') AS pohjus,;
iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tunnus)) = '1-1',;
'PEAPUHKUS', iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tunnus)) = '1-2',;
'STAAZ', iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tunnus)) = '1-3',;
'LASTEPUHKUS', iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tunnus)) = '1-4',;
'OMAARVEL', iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tunnus)) = '1-5',;
'ХPPEPUHKUS', iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tunnus)) = '2-1',;
'HAIGUS', iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tunnus)) = '2-2',;
'HOOLDUS',SPACE(20))))))))  AS liik ;
...
Function _alter_pg
	...
	If v_account.admin > 0 Or v_account.peakasutaja > 0
		...
		lcFile = 'tmp/fnc_muudatused20050202.txt'
