...
*!*	gnHandle2004 = SQLConnect('narvapg2004')
*!*	If gnHandle2004 < 1
...
lnError = SQLEXEC(gnHandle2005,lcString,'qryKONTOD')
If lnError < 0
	Messagebox('Viga,'+lcString)
	Set Step On
	Return lnError
Endif
lcstring = "select id, nimetus from rekv where id in (13, 31)"
lnError = SQLEXEC(gnHandle2005,lcString,'qryRekv')
If lnError < 0
	Messagebox('Viga,'+lcString)
	Set Step On
	Return lnError
...
Function f_loppsaldo
	...
	lcString = "select * from library where library = 'TUNNUS' and rekvid in (13,31)"
	lnError = SQLEXEC(gnHandle2005,lcString,'qryTunnus')
If lnError < 0
	Messagebox('Viga,'+lcString)
	Set Step On
	Return lnError
Endif
	...
	lcString = 'select l.kood, k.algsaldo from library l inner join kontoinf k on l.id = k.parentId '+;
		' where k.rekvid in (13,31) and algsaldo <> 0 order by kood '
	Wait Window 'Algsaldo konto' Nowait
	lnError = SQLEXEC(gnHandle2005,lcString,'qryAlgsaldo')
	If lnError < 0
		Messagebox('Viga,'+lcString)
		Set Step On
		Return lnError
	...
	lcString = 'select l.kood, k.algsaldo, k.asutusId from library l inner join subkonto k on l.id = k.kontoId '+;
		' where k.rekvid in (13,31) and algsaldo <> 0 and asutusid > 0 order by kood '

	Wait Window 'Algsaldo subkonto' Nowait

	lnError = SQLEXEC(gnHandle2005,lcString,'qryAlgSubsaldo')
	If lnError < 0
		Messagebox('Viga,'+lcString)
		Set Step On
		Return lnError
	...
	lcString = 'select l.kood, k.algsaldo, k.tunnusId from library l inner join tunnusinf k on l.id = k.kontoId '+;
		' where k.rekvid in (13,31) and algsaldo <> 0 and tunnusid > 0 order by kood '

	Wait Window 'Algsaldo tunnus' Nowait

	lnError = SQLEXEC(gnHandle2005,lcString,'qryAlgTunsaldo')
	If lnError < 0
		Messagebox('Viga,'+lcString)
		Set Step On
		Return lnError
	...
	lcString = 'select * from curjournal where kpv < DATE(2005,01,01) and rekvid in (13,31)'

	Wait Window 'kaibed' Nowait
	lnError = SQLEXEC(gnHandle2005,lcString,'qryKaibed')
	If lnError < 0
		Messagebox('Viga,'+lcString)
		Set Step On
		Return lnError
	...
	Scan
		...
		If Found()
			...
			lcString = "update kontoinf set algsaldo = "+ALLTRIM(Str(lnAlgsaldo,16,2))+;
				" where rekvid = 51 and parentId = "+Str(qryKONTOD.Id,9)
...
*!*			Wait WINDOW 'Updating sub'+ 
*!*			lcString = 'insert into subkonto (rekvid, algsaldo, kontoid, asutusId) '+;
*!*			' select distinct 51 as rekvid, 0 as algsaldo, kontoid, asutusId from subkonto where rekvid in (13,31) and STR(kontoid,12)+'-'+STR(asutusid,12) not in (select STR(kontoid,12)+'-'+STR(asutusid,12) from subkonto where rekvid = 51)'
	...
	Scan
		...
		If Found()
			...
			lcString = "update subkonto set algsaldo = "+ALLTRIM(Str(lnAlgsaldo,16,2))+;
				" where rekvid = 51 and kontoId = "+Str(qryKONTOD.Id,9)+" and asutusId = "+STR(qryLoppSubSaldo.asutusId,9)
			lnError = SQLEXEC(gnHandle2005,lcString)
			If lnError < 0
				Messagebox('Viga,'+lcString)
				Set Step On
	...
	Scan
		...
		If Found()
			...
			lcString = "update tunnusinf set algsaldo = "+ALLTRIM(Str(lnAlgsaldo,16,2))+;
				" where rekvid = 51 and kontoId = "+Str(qryKONTOD.Id,9)+" and tunnusId = "+STR(qryLoppTunSaldo.tunnusId,9)
			lnError = SQLEXEC(gnHandle2005,lcString)
			If lnError < 0
				Messagebox('Viga,'+lcString)
				Set Step On
