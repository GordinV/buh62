...
iif(alltrim(str(puudumine.tunnus)) + '-' + alltrim(str(puudumine.tyyp)) = '1-5', 'ХPPEPUHKUS', ;
...
SPACE(20)))))))))  AS liik ;
 FROM  buhdata5!puudumine  INNER JOIN buhdata5!tooleping  ON  Puudumine.lepingid = Tooleping.id   INNER JOIN buhdata5!library Amet  ON  Tooleping.ametid = Amet.id   INNER JOIN buhdata5!asutus  ON  Tooleping.parentid = Asutus.id
