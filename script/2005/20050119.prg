...
*!*	Select curPrinter
*!*	Locate For Lower(objekt) = LOWER('palkoper') And Id = 4
*!*	If !Found()
*!*	* lisamine
*!*		Append Blank
*!*		Replace Id With 4,;
...
Select curPrinter
Locate For Lower(objekt) = LOWER('aruanne') And Id = 202
If Found()
* lisamine
	Replace procfail With 'eelarve\saldoAsutusandmik_report2',;
		reportfail With 'eelarve\saldoAsutusandmik_report2',;
		reportvene With 'eelarve\saldoAsutusandmik_report2' In curPrinter
Endif
...
* lisamine
	Replace procfail With 'eelarve\saldoAsutusandmik_report2',;
		...
		reportvene With 'eelarve\saldoAsutusandmik_report2' In curPrinter
Endif
Locate For Lower(objekt) = LOWER('aruanne') And Id = 201
If Found()
* lisamine
	Replace procfail With 'eelarve\saldoAsutusandmik_report2',;
		reportfail With 'eelarve\saldoAsutusandmik_report2',;
	...
	Case gversia = 'VFP'
		...
		Scan For Mline(qryKey.Connect,1) = 'FOX'
			...
			If File (lcdata)
				...
				lcdataproc = lcdefault+'\tmp\buhdata5_20050119.sql'
				If File (lcdataproc)
...
*!*		Select menupohi
*!*		locate For Alltrim(Upper(Pad)) = Alltrim(Upper('Palkoper')) AND bar = '8'
*!*		If !Found()
*!*			lcOmandus = 'RUS CAPTION=Аванс'+Chr(13)+'EST CAPTION=Avans'
*!*			lcproc = "=nObjekt('do samm_avansitasu')"

*!*			Insert Into menupohi (Pad, Bar, proc_, omandus, level_) Values ;
*!*				('Palkoper','8',lcproc, lcOmandus, 2)
...
Function _alter_pg
	...
	lcFile = 'tmp/fnc_muudatused20050119.sql'
...
*!*		lcString = " select id from menupohi where UPPER(pad) = UPPER('Palkoper') and bar = '8'"
...
*!*				Return .F.
*!*			Endif
*!*			lcString = " select id from menupohi where UPPER(pad) = 'PALKOPER' and bar = '8'"
*!*			lError = SQLEXEC(gnhandle,lcString,'qrymenupohi')
