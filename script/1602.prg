...
CREATE SQL VIEW qryPvOper as;
SELECT pv_oper.parentId,  VAL(STR(00000000000.00,12,2)) as kulum, VAL(STR(00000000000.00,12,2)) as parandus, VAL(STR(00000000000.00,12,2)) as umberhind from pv_oper ;
	where  pv_oper.liik = 1;
union all;
SELECT pv_oper.parentId,  VAL(STR(sum(summa),12,2)) as kulum, VAL(STR(00000000000.00,12,2)) as parandus, VAL(STR(00000000000.00,12,2)) as umberhind from pv_oper ;
	where  pv_oper.liik = 2;
	AND pv_oper.kpv > getUmberhindlKpv(pv_oper.parentid);
	group by pv_oper.parentid;				
union all;
SELECT pv_oper.parentId, VAL(STR(00000000000.00,12,2)) as kulum, VAL(STR(sum(summa),12,2)) as parandus, VAL(STR(00000000000.00,12,2)) as umberhind from pv_oper ;
	where  pv_oper.liik = 3;
	AND pv_oper.kpv > getUmberhindlKpv(pv_oper.parentid) group by pv_oper.parentid;
union all;
SELECT pv_oper.parentId,  VAL(STR(00000000000.00,12,2)) as kulum, VAL(STR(00000000000.00,12,2)) as parandus, VAL(STR(sum(summa),12,2)) as umberhind from pv_oper ;
	where  pv_oper.liik = 5;
	group by pv_oper.parentid;				
