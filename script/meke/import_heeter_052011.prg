...
If !Used('heeter_052011')
	lcFile = 'c:\avpsoft\files\buh61\meke\heeter_052011.xls'
	Import From (lcFile) Type Xls
Endif
*SET STEP ON

Select heeter_052011
Scan For Upper(Alltrim(a)) <> 'HOTMETER'
* objekt

	lcObjekt = 	Alltrim(Upper(heeter_052011.a))
	If Atc('  ',lcObjekt) > 0
...
*	WAIT WINDOW lcObjekt
	...
	If lnResult > 0 And Reccount('qry1') > 0
		...
		lnObjektId = QRY1.Id
		lcKood = Upper(lcObjekt) + '-'+Alltrim(heeter_052011.b)
...
* kontrollin kas on andmed selle periodi jargi
		...
		Else
			If lnResult < 0
				...
				Exit
			Endif
			lcString = "select sp_salvesta_counter(0, "+Str(lnMotteId)+","+lcMotteKpv+","+heeter_052011.F+","+heeter_052011.e+",'import')"
			lnResult = SQLEXEC(gnHandle,lcString,'qry1')
...
* koef K
		...
		lcString = "update objekt set nait04 = "+heeter_052011.g + " where libId = "+ Str(lnObjektId)
