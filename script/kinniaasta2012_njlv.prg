...
*SET STEP ON
gnHandle2009 = SQLConnect('njlv2012')
gnHandle2008 = SQLConnect('njlv2011')
...
lcKpv = 'date(2012,01,01)'
lcKpv2 = 'date(2011,12,31)'
select qryRekv
scan
...
*		select * from qrySaldoKreedit1 into cursor qrySaldoKreedit
	...
	* kustutame vana lausend
	...
	lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2011,12,31) "
	...
	* Koostame deebet lausend
	...
	lcString = "insert into journal (rekvid, userid, asutusid, kpv, selg, muud ) values ("+;
		str(qryRekv.id)+",32,0,"+lcKpv2+",'Alg.saldo deebet','ALGSALDO')"	
...
*	brow
	scan
		...
		lcString = "insert into journal (rekvid, userid, asutusid, kpv,  selg,MUUD ) values ("+;
			str(qryRekv.id)+",32,"+str(qrySaldoDeebet.asutusId)+","+lcKpv2+",'Alg.saldo deebet','ALGSALDO')"	
...
lcString = "select Sp_salvesta_journal1(0,"+ str(qryId.id,9)+", "+str(ROUND(qrySaldoDeebet.algjaak/15.6466,2),14,2)+",'','','"+;
	  lcTegev+"','"+lcAllikas+"','"+ lcRahavoo+"','','','"+ qrySaldoDeebet.konto+"','"+lcTp+"','999999','"+lcTP+"','EUR',15.6466,"+;
	  str(ROUND(qrySaldoDeebet.algjaak/15.6466,2),14,2)+",'','')"
...
*!*				lcRahavoo+"','"+qrySaldoDeebet.konto+"','"+lcTp+"','999999','"+lcTp+"')"
	...
	* Koostame kreedit lausend
	...
	lcString = "insert into journal (rekvid, userid, asutusid, kpv,  selg,muud ) values ("+;
		str(qryRekv.id)+",32,0,"+lcKpv2+",'Alg.saldo kreedit','ALGSALDO')"	
...
lcString = "select Sp_salvesta_journal1(0,"+ str(qryId.id,9)+", "+str(ROUND(-1*qrySaldoKreedit.algjaak/15.6466,2),14,2)+",'','','"+;
	  lcTegev+"','"+lcAllikas+"','"+ lcRahavoo+"','','','999999','"+lcTp+"','"+qrySaldoKreedit.konto+"','"+lcTP+"','EUR',15.6466,"+;
