...
* asutuse nimekirje
lcString = "select id, nimetus from rekv where id = 6 and parentid <> 9999 and id <> 119 and parentid <> 119 " 
*!*	and "+;
	...
	* Tellime saldoandmik
	...
	If Used('qrySA')
		...
		lcString = "select konto, tp, tegev, allikas, rahavoo, sum(db) as db, sum(kr) as kr, nimetus from tmp_saldoandmik where rekvid = "+str(qryRekv.id)+;
			...
			" left(konto,3) not in ('102','103','109','108','201','202','203','206','208','258') "+;
...
*!*		endif
	...
	lcString = "select sp_subkontod_report('1020%',"+ Str(qryRekv.id)+","+lcKpv2+","+lcKpv2+",0,'%',3,0)"
	lError = sqlexec(gnhandle2008,lcString,'qrySA')
	tcTimestamp = Alltrim(qrySA.sp_subkontod_report)

	wait window lcAsutuseNimi+' subkontod 10...' nowait 
	
	lcString = "select algjaak, konto, asutusId from tmp_subkontod_report where left(konto,4) in('1020') and timestamp = '"+tcTimestamp+"' order by konto, asutusid" 
