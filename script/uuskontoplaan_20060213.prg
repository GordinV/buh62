...
If lError > 0
	...
	Select (lcCursor)
	Scan For !Empty(kontoplaan.b) AND !EMPTY(kontoplaan.C)
		Wait Window 'Import kontoplaan: kokku read:'+Str(Recno(lcCursor),9)+'-'+Str(Reccount(lcCursor),9) Nowait
* TAOTLEJA
* KONTROL
		Select qryKonto
		Locate For Alltrim(kood) = Alltrim(STR(kontoplaan.b,6))
		If !Found()
			Wait Window 'Konto puudub:'+STR(kontoplaan.b) Nowait
			lcString = "insert into library (rekvid, kood, nimetus, tun1, tun2, tun3, tun4, tun5, library) values (2,'"+;
				LTRIM(Rtrim(STR(kontoplaan.b,9)))+"','"+Ltrim(Rtrim(kontoplaan.c))+"',"+;
				"0,0,0,"+;
				IIF(!Empty(kontoplaan.g),"1","0")+",0,'KONTOD')"
