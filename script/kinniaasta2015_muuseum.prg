...
SET STEP ON
gnHandle2009 = SQLConnect('muuseum')
gnHandle2008 = SQLConnect('muuseum2015')
...
lcKpv = 'date(2016,01,01)'
lcKpv2 = 'date(2015,12,31)'
select qryRekv
scan
	...
	* Tellime saldoandmik
	...
	If Used('qrySA')
		...
		lcString = "select konto, tp, tegev, allikas, rahavoo, sum(db) as db, sum(kr) as kr, nimetus from tmp_saldoandmik where rekvid = "+str(qryRekv.id)+;
			...
			" left(konto,6) not in ('103000','201000') "+;
...
*	lcString = sqlexec(gnhandle2009,'begin work')
	...
	* kustutame vana lausend
	...
	lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2015,12,31) "
...
*!*		
	if (lnSummaDb - lnSummaKr) = 0
		wait window lcAsutuseNimi+' lõpp. edukalt' 
		*timeout 5
	endif
