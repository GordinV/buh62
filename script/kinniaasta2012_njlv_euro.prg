...
*SET STEP ON
gnHandle2009 = SQLConnect('njlv2013')
gnHandle2008 = SQLConnect('njlv2012')
...
lcKpv = 'date(2013,01,01)'
lcKpv2 = 'date(2012,12,31)'
select qryRekv
scan
	...
	* Tellime saldoandmik
	...
	If Used('qrySA')
		...
		lcString = "select konto, tp, tegev, allikas, rahavoo, sum(db) as db, sum(kr) as kr, nimetus from tmp_saldoandmik where rekvid = "+str(qryRekv.id)+;
			...
			" (left(konto,6) not in ('103000','103009','103610','103690','103699','103990') or "+;
			" LEFT(konto,3) not in ('201','202') ) "+;
...
*!*		lError = sqlexec(gnhandle2008,lcString,'qrySaldoDeebet1')
	...
	lcString = "select algjaak, konto, asutusId from tmp_subkontod_report where left(konto,6) in('103000','103009','103610','103690','103699','103990') and timestamp = '"+tcTimestamp+"' order by konto, asutusid" 
	lError = sqlexec(gnhandle2008,lcString,'qrySaldoDeebet')
...
*	brow
	...
	* kustutame vana lausend
	...
	lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2012,12,31) "
