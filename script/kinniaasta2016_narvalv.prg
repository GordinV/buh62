...
gnHandle2008 = SQLConnect('narvalv2015','vlad','Vlad490710')
If gnHandle2009 < 1
...
* asutuse nimekirje
lcString = "select id, nimetus from rekv where parentid < 999 and parentid <> 119 and id <> 119 and id <> 63" 
*!*	and "+;
...
scan
	...
	* Tellime saldoandmik
	...
	lcString  = " select sp_saldoandmik_report_recalc("+ Str(qryRekv.id)+","+lcKpv2+",2,0,0)"
	...
	If Used('qrySA')
		tcTimestamp = Alltrim(qrySA.sp_saldoandmik_report_recalc)
...
*		select * from qrySaldoKreedit1 into cursor qrySaldoKreedit
	...
	* kustutame vana lausend
	...
	lcString = "update aasta set kinni = 0 where rekvid = " + str(qryrekv.id,9)+ " and kuu = 12 and aasta = 2015"
	...
	lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2015,12,31) and selg ilike 'Alg.saldo%'"
	lError = sqlexec(gnhandle2009,lcString)
	if lError < 0
		messagebox('Viga.'+lcString)
		set step on
