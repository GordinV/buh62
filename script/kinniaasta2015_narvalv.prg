...
gnHandle2008 = SQLConnect('narvalv2014','vlad','Vlad490710')
If gnHandle2009 < 1
...
* asutuse nimekirje
lcString = "select id, nimetus from rekv where parentid < 999 and parentid <> 119 and id <> 119 and id = 3" 
*!*	and "+;
...
*		select * from qrySaldoKreedit1 into cursor qrySaldoKreedit
	...
	* kustutame vana lausend
	...
	lcString = "update aasta set kinni = 0 where rekvid = " + str(qryrekv.id,9)+ " and kuu = 12 and aasta = 2014"
	...
	lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2014,12,31) and selg ilike 'Alg.saldo%'"
	lError = sqlexec(gnhandle2009,lcString)
	if lError < 0
		messagebox('Viga.'+lcString)
		set step on
