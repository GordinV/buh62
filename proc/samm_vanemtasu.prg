...
Procedure arVutus
	...
	With odB
		...
		SELECT DISTINCT palklibid as nomid FROM curResult WHERE palklibid > 0 INTO CURSOR qryNomId
		.opEntransaction()
		...
		If leRror = .T.
			leRror = .cursorupdate('v_vanemtasu3')
		Endif
		IF lError = .t. and v_dokprop.registr > 0 AND v_vanemtasu3.Id > 0
			lError = .Exec("GEN_LAUSEND_KOOLITUS",Str(v_vanemtasu3.Id),'qryVanemTasuLausend')
		ENDIF
...
Procedure geT_isiku_list
	...
	If Reccount('curSource')>0
		Zap
	Endif
	Select koOd, LEFT(LTRIM(RTRIM(niMetus)),60)+'('+LTRIM(RTRIM(tunnus))+')' as nimetus, Id From qryLapsed;
