...
If Empty(tdKpv)
	tdKpv = GOMONTH(Date(YEAR(DATE()), MONTH(DATE()),1),1)-1
Endif
gdKpv = tdKpv
...
Procedure arVutus
	Local lError
	WAIT windows 'Koostan arve...' NOWAIT 
	...
	lError = oDb.readFromModel('hooldekodu\hooisik', 'koostaArved', 'l_params_id,guserid,gdKpv', 'v_tulemus')
	IF !lError
		MESSAGEBOX('Error',0+16, 'Arvete arvestus')
		SET STEP ON 
	ENDIF
	WAIT windows 'Arvestan arve...Ok' TIMEOUT 1	
