...
lcModel = 'libs\libraries\valuuta'
...
* expect LIBRARY
	...
	If Type('lcNotValidFields') = 'C'
		...
		Replace Id With 0, rekvid With gRekv, kood With '__test' + Left(Alltrim(Str(Rand() * 10000)),10),;
			...
			library With 'VALUUTA'
	ENDIF
	
	INSERT INTO v_valuuta1 (parentid, kuurs, alates, kuni) values (0, 1, DATE(), DATE() + 365)
...
* details
		...
		lError = oDb.readFromModel(lcModel, 'details', 'tnId, guserid', 'v_valuuta1')

		IF !lError or !USED('v_valuuta1')
			lError = .f.
		endif
