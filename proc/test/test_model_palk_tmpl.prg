...
LOCATE FOR !EMPTY(osakondid) AND id > 0
...
SET STEP ON 
	...
	lcNotValidFields = oDb.Validate(lcValidate, 'v_palk_tmpl')
* expect LIBRARY
	If Type('lcNotValidFields') = 'C'
		Replace Id With 0, ;
			parentid WITH comAmetRemote.id,;
			libid WITH comPalkLibRemote.id,;
			summa WITH 100 IN v_palk_tmpl
...
* cursor v_tunnus -> json
		...
		Select v_palk_tmpl
		lcJson = '{"id":' + Alltrim(Str(v_palk_tmpl.Id)) + ',"data":'+ oDb.getJson() + '}'
		lError = oDb.readFromModel(lcModel, 'saveDoc', 'lcJson,gUserid,gRekv', 'v_palk_tmpl_id')

		If 	!lError And Used('v_palk_tmpl_id') And Reccount('v_palk_tmpl_id') > 0 And !Empty(v_palk_tmpl_id.Id)
			Messagebox('test failed',0 + 48,'Error')
			Set Step On
			Return .F.
		Else
* success
			Wait Window 'test model ' + lcModel + ', saveDoc new -> passed' + Str(v_palk_tmpl_id.Id) Timeout 1
			Select v_palk_tmpl_id
			tnId = v_palk_tmpl_id.Id
			Use In v_palk_tmpl_id
			Return .T.
		Endif
...
Function test_of_row_new_model
	...
	With oDb
		lcAlias = 'row'
* parameters
		lError = oDb.readFromModel(lcModel, 'row', 'tnId, guserid', 'v_palk_tmpl')

		If 	!lError And Used('v_palk_tmpl') And Reccount('v_palk_tmpl') > 0
			Messagebox('test failed',0 + 48,'Error')
			Return .F.
		Endif
...
* parameters
		...
		If 	!lError And Used('v_palk_tmpl') And Reccount('v_palk_tmpl') > 0 And !Empty(v_palk_tmpl.Id)
			Messagebox('test failed',0 + 48,'Error')
			Return .F.
		Endif
...
Function test_of_grid_model
	With oDb
		...
		lcAlias = 'curPalkTmpl'
* parameters
		lError = oDb.readFromModel(lcModel, 'curPalkTmpl', 'gRekv, guserid', 'tmp', lcWhere)
		...
		Else
* success
			Wait Window 'test model ' + lcModel + ', curPalkTmpl -> passed' Timeout 3
			Use In tmp
			Return .T.
		Endif
	Endwith
