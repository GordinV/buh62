...
If gnHandle < 0
	...
	Return .T.
Endif
lcModel = 'libs\libraries\tahtpaev'
...
* parameters
		...
		Select(cursorName)
		If 	!lError  OR ! Used(cursorName) or Reccount(cursorName) = 0 or id = 0
			Messagebox('test failed',0 + 48,'Error')
			Set Step On
			Return .F.
		Else
...
Function test_of_row_model
	...
	aCheckedFields[1] = 'PAEV'
	aCheckedFields[2] = 'KUU'
	aCheckedFields[3] = 'LUHIPAEV'
