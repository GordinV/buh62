...
If lsuccess
	lsuccess = test_of_subkonto_model()
Endif
...
Function test_of_subkonto_model()
	With oDb
		lcAlias = 'subkonto'
		l_cursorName = 'v_subkonto'
		lError = oDb.readFromModel(lcModel, lcAlias, 'tnId, guserid', l_cursorName)
		If 	!lError And Used(l_cursorName)
...
* parameters
		...
		If 	!lError And Used(cursorName) And Reccount(cursorName) > 0 And Valid = Date(2018,12,31)
			Messagebox('test failed',0 + 48,'Error')
			Return .F.
		Else
* success
