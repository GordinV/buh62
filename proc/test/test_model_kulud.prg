...
IF !USED('comArtikkelRemote') OR !USED('comAllikadRemote') OR !USED('comTegevusRemote') OR !USED('comRahavoogRemote')
	MESSAGEBOX('test fail, can not load libs',0+16,'Test faild')
	SET STEP ON
	RETURN .t.
ENDIF
...
* expect LIBRARY
	...
	If Type('lcNotValidFields') = 'C'
		...
		Replace Id With 0, rekvid With gRekv, ;
			kood1 With comTegevusRemote.kood,;
			kood2 With comAllikadRemote.kood,;
			kood3 With comRahavoogRemote.kood,;
			kood5 With comArtikkelRemote.kood,;
			summa With 100,;
			is_kulud with 1,;
			aasta with 2018,;
			muud with 'test' 
