...
l_nom_id = getNomIdByNimetus('OMNIVA')
If Empty(l_nom_id)
...
If !Empty(v_xml_arv.korr_konto)
	t_dokprop_id = getdokpropId('ARV', 'libs\libraries\dokprops', v_xml_arv.korr_konto)
Endif
...
* seach for noms
		...
		Insert Into v_arvread (nomid, kogus, hind, Summa, kbmta, km, kbm, nimetus, konto, tp, kood1, kood2, kood5, MUUD,;
			tunnus, Proj, kood4, kood3, objekt) ;
			...
			v_xml_arv_detail.nimetus,;
			v_xml_arv_detail.tunnus, v_xml_arv_detail.Proj, v_xml_arv_detail.uuritus, v_xml_arv_detail.rahavoog, v_xml_arv_detail.objekt)
	Endscan
